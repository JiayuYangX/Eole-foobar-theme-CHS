    ...
    this.draw = function(gr) {
        if(this.offset > 0) {
           ...
           // gr.FillSolidRect(cx + bg_margin_left, this.y + cPlaylistManager.topbarHeight - 2, this.w - bg_margin_left*2, 1, colors.pm_bg4);
            ...
            if(cPlaylistManager.blink_counter > -1) {
                if(cPlaylistManager.blink_row == 0) {
					gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - this.tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                    if(cPlaylistManager.blink_counter <= 6 && Math.floor(cPlaylistManager.blink_counter / 2) == Math.ceil(cPlaylistManager.blink_counter / 2)) {
                    ...
                    gr.GdiDrawText("发送到 ..." , g_font.normal, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - this.tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                };
            } else {
                if(this.activeRow == 0) {
                    gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - this.tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
...
function panel_operations() {
	...
	if(!ViewIsInited)
	{
		window.SetProperty("View by Folder Structure: Name // Pattern", "按文件夹结构查看 // 不能配置模板");
		...
		window.SetProperty("View 03: Name // Pattern", "按专辑艺术家 - 专辑 // [%album artist% - ]['['%date%']' ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%");
		window.SetProperty("View 04: Name // Pattern", "按专辑 // %album%[ '['%album artist%']']|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%");
		window.SetProperty("View 05: Name // Pattern", "按流派 // %genre%|[%album artist% - ][(%date%) ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%");
		window.SetProperty("View 06: Name // Pattern", "按年份 // %date%|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%");
		
		window.SetProperty("View Filter 01: Name // Query", "否 // 不能配置查询");
		window.SetProperty("View Filter 02: Name // Query", "无损 // \"$info(encoding)\" IS lossless");
		window.SetProperty("View Filter 03: Name // Query", "有损 // \"$info(encoding)\" IS lossy");
		window.SetProperty("View Filter 04: Name // Query", "无播放增益 // %replaygain_track_gain% MISSING");
		window.SetProperty("View Filter 05: Name // Query", "从未播放 // %play_count% MISSING");
		window.SetProperty("View Filter 06: Name // Query", "经常播放 // %play_count% GREATER 9");
		window.SetProperty("View Filter 07: Name // Query", "最近添加 // %added% DURING LAST 2 WEEKS");
		window.SetProperty("View Filter 08: Name // Query", "最近播放 // %last_played% DURING LAST 2 WEEKS");
		window.SetProperty("View Filter 09: Name // Query", "最高评级 // " + (globalProperties.use_ratings_file_tags ? "$meta(rating)" : "%rating%") + " GREATER 3");
		
		window.SetProperty("View: View Is Inited", true);
	}
	
	
    var def_ppt = window.GetProperty("View by Folder Structure: Name // Pattern", "按文件夹结构查看 // 不能配置模板");
    ...
    var view_ppt = [
        ...
        window.GetProperty("View 03: Name // Pattern", "按专辑艺术家 - 专辑 // [%album artist% - ]['['%date%']' ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"),
        window.GetProperty("View 04: Name // Pattern", "按专辑 // %album%[ '['%album artist%']']|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"),
        window.GetProperty("View 05: Name // Pattern", "按流派 // %genre%|[%album artist% - ][(%date%) ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%"),
        window.GetProperty("View 06: Name // Pattern", "按年份 // %date%|[%album artist% - ]%album%|[[%discnumber%.]%tracknumber%. ][%track artist% - ]%title%")
    ];
    var nm = "",
    ...
    var filter_ppt = [
        window.GetProperty("View Filter 01: Name // Query", "否 // 不能配置查询"),
        ...
        window.GetProperty("View Filter 04: Name // Query", "无播放增益 // %replaygain_track_gain% MISSING"),
        ...
        window.GetProperty("View Filter 09: Name // Query", "最高评级 // " + (globalProperties.use_ratings_file_tags ? "$meta(rating)" : "%rating%") + " GREATER 3")
    ];
...
function library_manager() {
    ...
    this.rootNodes = function(callID) {
        ...
        console.log("--> 树形媒体库填充: " + this.time.Time / 1000 + " 秒数 call_id:"+callID);
...
function populate() {
    ...
    if (!window.GetProperty("SYSTEM.Playlist Checked", false)) fb.ShowPopupMessage("树形媒体库默认使用以下播放列表:\n\n选择媒体库\n\n如果您想使用一个不同的播放列表, 请在面板属性中改变树形媒体库.", "树形媒体库");
    ...
    this.draw = function(gr) {
        ...
        if (!pop.tree.length) { // no track, tree is empty
			if(p.s_txt.length>0){
				...
				var line_width = Math.min(130,Math.round(ui.w-40));
				var py = p.s_h + Math.round((ui.h-p.s_h)  / 2)-1;
				gr.GdiDrawText("正在载入...", g_font.plus3, colors.normal_txt, px, py - 40, ui.w, 36, DT_CENTER | DT_BOTTOM | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
				gr.FillSolidRect(px+Math.round(ui.w/2-line_width/2),py, line_width, 1, colors.border);
				gr.GdiDrawText("树形媒体库", g_font.italicplus1, colors.faded_txt, px, py + 6, ui.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
				return;
				//return gr.GdiDrawText(lib.none, ui.font, ui.textcol, ui.margin, p.s_h, ui.w - p.r_mg, ui.row_h * (lib.none.length > 14 ? 5 : 1), 0x00000004 | 0x00000400);
			}
		}
    ...
    this.move = function(x, y) {
		...
		if(ui.drag_clicked && !ui.drag_moving && properties.DropInplaylist && !(sbar.hover || sbar.b_is_dragging) && !p.s_search && m_i>=0) {
			if((Math.abs(x - ui.drag_clicked_x) > 10 || Math.abs(y - ui.drag_clicked_y) > 10) && ui.h > cPlaylistManager.rowHeight * 6 && pop.sel_items.length>0) {
				...
				var options = {
					...
					use_album_art : false,
					use_theming : false,
					custom_image : createDragText(drag_text, items.Count+" 音轨", 220),
				}
        ...
        if (ix != -1) {
            ...
            if (p.tooltip && typeof this.tree[ix] !== "undefined"  && tt_id != ix && Math.round(ui.pad * this.tree[ix].tr + ui.margin) + (ui.node_style==0 && this.tree[ix].track ? 0 : ui.symbol_w) + this.tree[ix].w > ui.w - p.r_mg && !(sbar.hover || sbar.b_is_dragging) && !hover_node) {
				...
				if(g_tooltip.getActiveZone()!='切换分支'+ix){
					g_tooltip.Deactivate();
					g_tooltip.ActivateDelay((this.tree[ix].child.length < 1?"展开分支":"折叠分支"), x+10, y+20, globalProperties.tooltip_button_delay, 1200, false, "切换分支"+ix);
				}
			}
...
function searchLibrary() {
    ...
    this.draw = function(gr) {
       ...
       // gr.DrawLine(0, this.y+p.s_sp, ww -((draw_right_line)?2:1), this.y+p.s_sp, 1,colors.headerbar_line);
        ...
        } else if(!(p.s_search)) gr.GdiDrawText("过滤...", g_font.plus1, colors.normal_txt, p.s_x, this.y, p.s_w2-5, p.s_sp-1, p.l | DT_END_ELLIPSIS);
        sL.drawcursor(gr);
...
function button_manager() {
    ...
    var browser = function(c) {
        if (!run(c)) fb.ShowPopupMessage("无法启动您的默认浏览器.", "树形媒体库");
    }
    var run = function(c) {
        try {
...
function menu_object() {
    ...
    this.PlaylistTypeMenu = function(Menu, StartIndex) {
        var Index = StartIndex,
            n = ["发送到当前播放列表", "插入到当前播放列表", "添加到当前播放列表"];
        for (i = 0; i < 3; i++) {
            this.NewMenuItem(Index, "播放列表", i + 1);
    ...
    this.OptionsTypeMenu = function(Menu, StartIndex,reduced) {
        var Index = StartIndex,
            mt = p.syncType ? 1 : 0;
		if(!reduced){
			Menu.AppendMenuItem(MF_STRING, 7000, "切换到过滤");
			Menu.AppendMenuItem(MF_SEPARATOR, 0, 0);

			var menuDisplay = window.CreatePopupMenu();

			menuDisplay.AppendMenuItem(MF_STRING, 7017, "标签切换栏");
			menuDisplay.CheckMenuItem(7017,p.tag_switcherbar)
			menuDisplay.AppendMenuItem(MF_STRING, 7018, "隐藏菜单按钮");
			menuDisplay.CheckMenuItem(7018,p.showFiltersTogglerBtn)
			var searchbar = window.CreatePopupMenu();
			searchbar.AppendMenuItem(MF_STRING, 7005, "隐藏");
			searchbar.AppendMenuItem(MF_STRING, 7006, "仅搜索栏");
			searchbar.AppendMenuItem(MF_STRING, 7007, "搜索栏 + 按钮设置");
			searchbar.CheckMenuRadioItem(7005, 7007, (p.s_show==0) ? 7005 : (p.s_show==1) ? 7006 : 7007);
			searchbar.AppendTo(menuDisplay, MF_STRING, "搜索栏");

			menuDisplay.AppendMenuItem(MF_SEPARATOR, 0, 0);

			var nodeStyle = window.CreatePopupMenu();
			nodeStyle.AppendMenuItem(MF_STRING, 7010, "+/- 分支");
			nodeStyle.AppendMenuItem(MF_STRING, 7011, "方形分支");
			nodeStyle.AppendMenuItem(MF_STRING, 7012, "三角分支");
			nodeStyle.CheckMenuRadioItem(7010, 7012, (ui.node_style==0) ? 7010 : (ui.node_style==1) ? 7011 : 7012);
			nodeStyle.AppendTo(menuDisplay, MF_STRING, "分支样式");

			var lineStyle = window.CreatePopupMenu();
			lineStyle.AppendMenuItem(MF_STRING, 7030, "无线");
			lineStyle.AppendMenuItem(MF_STRING, 7031, "音轨线");
			lineStyle.AppendMenuItem(MF_STRING, 7032, "全部线");
			lineStyle.CheckMenuRadioItem(7030, 7032, (ui.linestyle==0) ? 7030 : (ui.linestyle==1) ? 7032 : 7031);
			lineStyle.AppendTo(menuDisplay, MF_STRING, "线样式");

			menuDisplay.AppendMenuItem(MF_STRING, 7009, "项目合计");
			menuDisplay.CheckMenuItem(7009,pop.show_aggregate_item)
			menuDisplay.AppendMenuItem(MF_STRING, 7008, "项目数量显示");
			menuDisplay.CheckMenuItem(7008,pop.show_counts)
			menuDisplay.AppendMenuItem(MF_STRING, 7016, "显示工具条提示");
			menuDisplay.CheckMenuItem(7016,p.tooltip)

			menuDisplay.AppendTo(Menu,MF_STRING, "显示");

			var rowPadding = window.CreatePopupMenu();
			rowPadding.AppendMenuItem(MF_STRING, 7020, "增加");
			rowPadding.AppendMenuItem(MF_STRING, 7021, "减少");
			rowPadding.AppendMenuItem(MF_SEPARATOR, 0, 0);
			rowPadding.AppendMenuItem(MF_DISABLED, 0, "提示: 按住 SHIFT 并使用鼠标滚轮");
			rowPadding.AppendMenuItem(MF_DISABLED, 0, "在皮肤面板的任何位置滚动滚轮 !");
			...
			_panelWidth.AppendTo(Menu,MF_STRING, "面板宽度");

			var wallpaper = window.CreatePopupMenu();
			wallpaper.AppendMenuItem(MF_STRING, 9000, "启用");
			wallpaper.CheckMenuItem(9000, properties.showwallpaper);
			wallpaper.AppendMenuItem(MF_STRING, 9020, "模糊");
			wallpaper.CheckMenuItem(9020, properties.wallpaperblurred);
			var wallpaper2 = window.CreatePopupMenu();
			wallpaper2.AppendMenuItem(MF_STRING, 9021, "填充");
			wallpaper2.CheckMenuItem(9021, properties.wallpaperdisplay==0);
			wallpaper2.AppendMenuItem(MF_STRING, 9022, "适合");
			wallpaper2.CheckMenuItem(9022, properties.wallpaperdisplay==1);
			wallpaper2.AppendMenuItem(MF_STRING, 9023, "拉伸");
			wallpaper2.CheckMenuItem(9023, properties.wallpaperdisplay==2);
			wallpaper2.AppendTo(wallpaper,MF_STRING, "壁纸大小");
			wallpaper.AppendTo(Menu,MF_STRING, "壁纸背景");

			Menu.AppendMenuItem(MF_SEPARATOR, 0, 0);

			Menu.AppendMenuItem(MF_STRING, 7015, "自动折叠");
			Menu.CheckMenuItem(7015,pop.auto);
			Menu.AppendMenuItem(MF_STRING, 7023, "自动展开子项");
			Menu.CheckMenuItem(7023,pop.autoExpandSingleChild);
		} else {
			Menu.AppendMenuItem(MF_STRING, 7024, "设置...");
		}
		Menu.AppendMenuItem(MF_SEPARATOR, 0, 0);
        for (i = 0; i < p.menu.length; i++) {
			if(p.menu[i]!="面板属性" && p.menu[i]!="配置..."){
				this.NewMenuItem(Index, "选项", i + 1);
    ...
    this.FilterMenu = function(Menu, StartIndex) {
        ...
        for (i = 0; i < p.f_menu.length; i++) {
            this.NewMenuItem(Index, "过滤", i + 1);
            Menu.AppendMenuItem(MF_STRING, Index, i != p.f_menu.length ? (!i ? "不过滤" : ""+ p.f_menu[i]) : "重置滚动");
	...
	this.rbtn_down = function(x, y) {
        ...
        if (y > p.s_h && pop.tree.length > ix && ix >= 0 && pop.check_ix(pop.tree[ix], x, y, false)) {
            menu.AppendMenuItem(MF_STRING, 7004, "设置...");
            menu.AppendMenuSeparator();
            menu.AppendMenuItem(MF_STRING, 7006, "定位到播放分组");
            menu.AppendMenuSeparator();
			...
			if(ui.show_collapse_rclick && !pop.auto){
				menu.AppendMenuItem(MF_STRING, 7005, "全部折叠");
				menu.AppendMenuSeparator();
			}
            if (!pop.tree[ix].sel) {
                new_sel = true;
			...
			if(pop.sel_items.length>0) {
				...
				for (var i = 0; i < list.length; i++) items.Add(p.list[list[i]]);
				this.metadblist_selection = items;
				sendTo.AppendTo(menu, MF_STRING, "发送到...");
				sendTo.AppendMenuItem(MF_STRING, 10000, "新播放列表...");
        ...
        if (show_context) {
			...
			if(show_open_folder && p.view_by == p.folder_view) {
				menu.AppendMenuSeparator();
				menu.AppendMenuItem(MF_STRING, 9999, "打开文件夹");
			}
        } else showOptionsMenu(x,y,false);

		if(utils.IsKeyPressed(VK_SHIFT)) {
			menu.AppendMenuSeparator();
			menu.AppendMenuItem(MF_STRING, 7001, "属性");
			menu.AppendMenuItem(MF_STRING, 7002, "配置...");
			menu.AppendMenuSeparator();
			menu.AppendMenuItem(MF_STRING, 7003, "重载入");
		}
...
function showOptionsMenu(x,y,reduced){
	...
	menuFilter.AppendTo(menu, (p.filter_by==0)?MF_STRING:MF_CHECKED, "过滤方式");
...
function get_images() {
	...
	reset_bt = new button(images.resetIcon_off, images.resetIcon_ov, images.resetIcon_dn,"reset_bt", "重置过滤");
};
