...
var properties = {
    ...
    TFgrouping_default_filterbox: "%album artist% ^^ %album%$ifgreater(%totaldiscs%,1,[' - 光碟 '%discnumber%],) ^^ %genre% ^^ %date% ^^ %title%",
    TFgrouping_default: "%album artist% ^^ %album%$ifgreater(%totaldiscs%,1,[' - 光碟 '%discnumber%],)",
    ...
    TFtitle: "%artist% ^^ [%discnumber%.] ^^ %tracknumber% ^^ %title% ^^ $if2(" + (globalProperties.use_ratings_file_tags ? "$meta(rating)" : "%rating%") + ",0) ^^ $if(%length%,%length_seconds%,'广播')",
    TFbitrate: "$if2(%bitrate% kbit,'')",
    TFcodec: "$if2(%codec%,'')",
    TFplaycount: "$if2(%play_counter%,$if2(%play_count%,0)) plays",
    TFshowlist: "%album artist% ^^ %album% ^^ $ifgreater(%totaldiscs%,1,[' - 光碟 '%discnumber%],) ^^ %date% ^^ %genre%",
...
var TF = {
	...
	meta_changed: fb.TitleFormat("$if2(%album artist%,'未知艺术家') ^^ $if2(%album%,'单曲')")
}
...
oPlaylistHistory = function() {
	...
	this.trace = function() {
		for(i = 1; i < this.history.length; i++) {
			console.log('历史 '+i+':'+plman.getPlaylistName(this.history[i]));
		}
	}
...
oFilterBox = function() {
	...
	this.getImages = () => {
		...
		this.search_bt = new button(this.images.search_icon, this.images.search_icon, this.images.search_icon,"search_bt", "过滤分组"+(properties.filterBox_filter_tracks?" & 音轨":''));
		...
		if(typeof(this.reset_bt) == "undefined") {
			this.reset_bt = new button(this.images.resetIcon_off, this.images.resetIcon_ov, this.images.resetIcon_dn,"reset_bt", "重置过滤");
		} else {
	...
	this.on_init = () => {
		...
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", "过滤下列分组 ...", colors.normal_txt, 0, 0, colors.selected_bg, g_sendResponse, "brw", undefined, "g_font.italicplus2");
...
oPlaylistManager = function (parentObjName) {
    ...
    this.draw = function(gr) {
		...
		if(this.ishoverHeader)
			gr.GdiDrawText("创建播放列表", g_font.normal, colors.normal_txt, this.x+20, this.y+17, this.w-20, this.headerHeight, DT_VCENTER | DT_NOPREFIX);
		else
			gr.GdiDrawText("发送到 ...", g_font.italicplus1, colors.normal_txt, this.x+20, this.y+17, this.w-20, this.headerHeight, DT_VCENTER | DT_NOPREFIX);
...
oRow = function(metadb,itemIndex) {
	...
	this.getTags = function() {
		...
		this.artist = Tags[0];
		if(this.artist=="?") this.artist="未知艺术家";
  ...
  this.draw = (gr, x, y, w) => {
		...
		if((properties.showArtistName || (properties.TFgrouping!="" && properties.TFgrouping.indexOf("artist%")==-1) || (this.artist!=brw.groups[g_showlist.idx].artist && this.artist!="未知艺术家"))) this.artist_text = this.artist;
            ...
            case "dblclk":
                if(this.ishover) {
					if(!getRightPlaylistState() || brw.SourcePlaylistIdx==plman.PlayingPlaylist){
						...
						fb.RunContextCommandWithMetadb("添加到播放队列", this.metadb);
						fb.Play();
					} else {
...
oShowList = function(parentPanelName) {
	...
	this.cover_y = -1;
	this.links = {
		album : new SimpleButton(0, 0, 0, 0, "albumLink", "显示专辑", function () {
			scroll = scroll_ = 0;
			quickSearch(g_showlist.pl[0],"album");
		},false,false,false,ButtonStates.normal,255),
		artist : new SimpleButton(0, 0, 0, 0, "artistLink", "显示全部艺术家", function () {
			scroll = scroll_ = 0;
			quickSearch(g_showlist.pl[0],"artist");
		},false,false,false,ButtonStates.normal,255),
		genre : new SimpleButton(0, 0, 0, 0, "genreLink", "显示全部流派", function () {
			scroll = scroll_ = 0;
    ...
    this.setCloseButton = function (save_btns) {
		...
		if(!this.close_bt) {
			...
			if(typeof(this.close_bt) == "undefined") {
				this.close_bt = new button(this.closeTracklist_off, this.closeTracklist_ov, this.closeTracklist_ov,"showlist_close","关闭音轨列表");
			} else {
    ...
    this.setColumnsButtons = function (save_btns) {
		...
		if(!this.next_bt || !this.prev_bt) {
			...
			if(typeof(this.prev_bt) == "undefined") {
				this.prev_bt = new button(this.prevColumn_off, this.prevColumn_ov, this.prevColumn_ov,"showlist_prev","显示前面音轨");
			} else {
			...
			if(typeof(this.next_bt) == "undefined") {
				this.next_bt = new button(this.nextColumn_off, this.nextColumn_ov, this.nextColumn_ov,"showlist_next","显示后面音轨");
			} else {
    ...
    this.getHeaderInfos = function(EvalWithMetadb) {
		...
		if(EvalWithMetadb){
			...
			this.date = Tags[3];
			this.genre = Tags[4];
			this.total_tracks = pl_count+(pl_count>1?" 音轨":" 音轨");
		} else {
			...
			//this.discnumber = (Tags[0]=='')?'':' - 光碟 '+Tags[0];
			this.discnumber = '';
...
oHeaderBar = function(name) {
    ...
    this.setHideButton = function () {
		...
		if(typeof(this.hide_filters_bt) == "undefined") {
			this.hide_filters_bt = new button(this.hide_bt_off, this.hide_bt_ov, this.hide_bt_ov,"hide_filters", "显示左侧菜单");
		} else {
    ...
    this.setButtons = function () {
		...
		if(typeof(this.FullLibraryButton) == "undefined") {
			this.FullLibraryButton = new button(this.full_library_off, this.full_library_hover, this.full_library_off,"fulllibrary","显示全部媒体库");
		} else {
		...
		if(typeof(this.GridModeButton) == "undefined") {
			if(properties.CoverGridNoText)
				this.GridModeButton = new button(this.grid_mode_on, this.grid_mode_on_hover, this.grid_mode_on,"gridmode","显示模式");
			else if(properties.circleMode)
				this.GridModeButton = new button(this.grid_mode_off, this.grid_mode_off_hover, this.grid_mode_off,"gridmode","显示模式");
			else
				this.GridModeButton = new button(this.grid_mode_off_circle, this.grid_mode_off_circle_hover, this.grid_mode_off_circle,"gridmode","显示模式");
		} else {
			if(properties.CoverGridNoText){
		...
		if(typeof(this.SettingsButton) == "undefined") {
			this.SettingsButton = new button(this.settings_off, this.settings_hover, this.settings_off,"settings_bt","设置...");
		} else {
		...
		if(brw.finishLoading) {
			if(brw.playlistItemCount) {
				if(g_filterbox.isActive) {
					this.itemsTxt=((properties.showTotalTime)?',  ':'')+g_filterbox.filtered_tracks_count+' 音轨'+((g_filterbox.filtered_tracks_count>1)?" ":"")+',  '+brw.groups_draw.length+' 分组 '+((brw.groups_draw.length>1)?" ":"");
					time_total = g_filterbox.filtered_tracks_time;
				} else {
					this.itemsTxt=((properties.showTotalTime)?',  ':'')+brw.playlistItemCount+' 音轨'+((brw.playlistItemCount>1)?" ":"")+',  '+brw.groups_draw.length+' 分组 '+((brw.groups_draw.length>1)?" ":"");
				...
				if(brw.playlistName==globalProperties.whole_library){
					...
					this.mainTxt='播放列表 : '+brw.playlistName;
	...
	//	if(!plman.IsAutoPlaylist(plman.ActivePlaylist)){
			...
			SortMenu.AppendTo(basemenu, MF_STRING, "排序按");

			SortMenu.AppendMenuItem(MF_STRING, 2999, "不排序 (播放顺序)");
			SortMenu.AppendMenuSeparator();
			SortMenu.AppendMenuItem(MF_STRING, 3000, "艺术家/专辑/音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 3001, "标题");
			SortMenu.AppendMenuItem(MF_STRING, 3002, "音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 3003, "日期");
			SortMenu.AppendMenuItem(MF_STRING, 3004, "加入媒体库日期 (新加入优先)");
			SortMenu.AppendMenuItem(MF_STRING, 3005, "音轨评级");			
			SortMenu.AppendMenuItem(MF_STRING, 3006, "自定义标题格式...");
			...
			SortMenu.AppendMenuItem(MF_STRING, 3007, "随机");
			SortMenu.AppendMenuSeparator();
			SortMenu.AppendMenuItem(MF_STRING, 3008, "反向排序");
			SortMenu.CheckMenuItem(3008, properties.SortDescending);
			SortMenu.AppendMenuSeparator();
			SortMenu.AppendMenuItem(MF_STRING, 3009, "当前排序设为默认");
			...
			actions[3006] = function(){
				try {
					new_TFsorting = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.", "自定义排序", brw.currentSorting, true);
	...
	this.append_group_menu = function(basemenu,actions){
		...
		GroupMenu.AppendTo(basemenu, MF_STRING, "分组按");

		GroupMenu.AppendMenuItem(MF_STRING, 4000, "默认 (专辑, 艺术家)");
		GroupMenu.AppendMenuItem(MF_STRING, 4001, "自定义标题格式...");
		GroupMenu.AppendMenuSeparator();
		GroupMenu.AppendMenuItem(MF_STRING, 4002, "合并多张碟片专辑音轨");
		...
		actions[4001] = function(){
			try {
				...
				customGraphicBrowserGrouping("自定义分组"
									,"<div class='titleBig'>自定义分组</div><div class='separator'></div><br/>为分组的每行文本输入标题格式脚本. 注意不会对音轨进行排序, 将在定义相同的 2 行中对两个连续音轨进行分组.\n您可以在这里使用完整的 foobar2000 标题格式语法.<br/><a href=\"http://tinyurl.com/lwhay6f\" target=\"_blank\">点击这里</a> 了解关于工具栏标题格式信息. (http://tinyurl.com/lwhay6f)<br/>"
									,''
									,'分组第一行 (默认为 %album artist%):##第二行 (默认为 %album%, 留空以显示音轨数量):'
	...
	this.append_properties_menu = function(basemenu,actions){
		...
		if(fb.IsPlaying) basemenu.AppendMenuItem(MF_STRING, 802, "显示正在播放");
		basemenu.AppendMenuItem(MF_STRING, 803, "全部播放");
		basemenu.AppendMenuSeparator();

		//basemenu.AppendMenuItem(MF_STRING, 801, "音轨属性");
		actions[801] = function(){
			fb.RunContextCommandWithMetadb("属性", plman.GetPlaylistItems(brw.getSourcePlaylist()), 0);
		}
	...
	this.draw_header_menu = function(x,y,right_align){
		...
		basemenu.AppendMenuItem(MF_STRING, 1, "设置...");
		basemenu.AppendMenuSeparator();
		...
		if(utils.IsKeyPressed(VK_SHIFT)) {
			basemenu.AppendMenuSeparator();
			basemenu.AppendMenuItem(MF_STRING, 3100, "属性 ");
			basemenu.AppendMenuItem(MF_STRING, 3101, "配置...");
			basemenu.AppendMenuSeparator();
			basemenu.AppendMenuItem(MF_STRING, 3102, "重载入");
		}
...
function draw_settings_menu(x,y,right_align,sort_group){
	...
	_menu.AppendMenuItem(MF_STRING, 10, "关注正在播放 (隐藏右侧播放列表)");
	_menu.CheckMenuItem(10, properties.followNowPlaying);
	_menu.AppendMenuItem(MF_STRING, 31, "显示工具条提示");
	_menu.CheckMenuItem(31, properties.showToolTip);

	_NowPlaying.AppendMenuItem(MF_GRAYED, 0, "显示右侧播放列表:");
	_NowPlaying.AppendMenuSeparator();
	_NowPlaying.AppendMenuItem(MF_STRING, 51, "显示媒体库");
	_NowPlaying.AppendMenuItem(MF_STRING, 52, "显示播放中的播放列表");
	_NowPlaying.CheckMenuRadioItem(51, 52, (properties.showInLibrary_RightPlaylistOn) ? 51 : 52);
	_NowPlaying.AppendMenuSeparator();
	_NowPlaying.AppendMenuItem(MF_GRAYED, 0, "隐藏右侧播放列表:");
	_NowPlaying.AppendMenuSeparator();
	_NowPlaying.AppendMenuItem(MF_STRING, 53, "显示媒体库");
	_NowPlaying.AppendMenuItem(MF_STRING, 54, "显示播放中的播放列表");
	_NowPlaying.CheckMenuRadioItem(53, 54, (properties.showInLibrary_RightPlaylistOff) ? 53 : 54);
	_NowPlaying.AppendTo(_menu,MF_STRING, '"显示正在播放" 行为');

	_menu.AppendMenuSeparator();
	_menuHeaderBar.AppendMenuItem(MF_STRING, 27, "显示");
	_menuHeaderBar.CheckMenuItem(27, properties.showheaderbar);
	_menuHeaderBar.AppendMenuSeparator();

	_filterMenu.AppendMenuItem((!properties.showheaderbar)?MF_DISABLED:MF_STRING, 30, "显示")
	_filterMenu.CheckMenuItem(30, properties.showFilterBox);
	_filterMenu.AppendMenuItem(MF_STRING, 50, "过滤 also 音轨 (稍微减慢搜索速度)");
	_filterMenu.CheckMenuItem(50, properties.filterBox_filter_tracks);
	_filterMenu.AppendTo(_menuHeaderBar,MF_STRING, "过滤栏");

	_menuHeaderBar.AppendMenuItem((!properties.showheaderbar)?MF_DISABLED:MF_STRING, 41, "显示总时间");
	_menuHeaderBar.CheckMenuItem(41, properties.showTotalTime);
	_menuHeaderBar.AppendMenuItem((!properties.showheaderbar)?MF_DISABLED:MF_STRING, 42, "显示封面调整");
	_menuHeaderBar.CheckMenuItem(42, properties.showCoverResizer);
	_menuHeaderBar.AppendMenuItem((!properties.showheaderbar)?MF_DISABLED:MF_STRING, 55, "显示播放模式按钮");
	_menuHeaderBar.CheckMenuItem(55, properties.showGridModeButton);
	_menuHeaderBar.AppendTo(_menu,MF_STRING, "标题栏");

	_menuFilters.AppendMenuItem(MF_STRING, 40, (!libraryfilter_state.isActive())?"显示":"隐藏");
	_menuFilters.AppendMenuItem(MF_STRING, 39, "显示切换按钮");
	_menuFilters.CheckMenuItem(39, properties.displayToggleBtns);
	_menuFilters.AppendTo(_menu,MF_STRING, "左侧菜单");

	_menuGroupDisplay.AppendMenuItem(MF_STRING, 100, "方形封面");
	_menuGroupDisplay.AppendMenuItem(MF_STRING, 101, "圆形封面");
	_menuGroupDisplay.AppendMenuItem(MF_STRING, 102, "方形模式, 无填充, 无标签");
	...
	_dateMenu.AppendMenuItem(MF_STRING, 25, "显示");
	_dateMenu.CheckMenuItem(25, properties.showdateOverCover);
	_dateMenu.AppendMenuItem(MF_STRING, 49, "尝试提取并仅显示年份");
	_dateMenu.CheckMenuItem(49, properties.extractYearFromDate);
	_dateMenu.AppendTo(_menuGroupDisplay,MF_STRING, "专辑封面显示日期");

	_menuGroupDisplay.AppendMenuItem(MF_STRING, 26, "专辑封面显示碟片编号");
	_menuGroupDisplay.CheckMenuItem(26, properties.showDiscNbOverCover);
	_menuGroupDisplay.AppendMenuItem(MF_STRING, 46, "动画同时显示正在播放");
	_menuGroupDisplay.CheckMenuItem(46, properties.animateShowNowPlaying);
	_menuGroupDisplay.AppendMenuSeparator();

	_menuGroupDisplay.AppendMenuItem((properties.CoverGridNoText | properties.circleMode)?MF_GRAYED:MF_STRING, 38, "文本居中");
	_menuGroupDisplay.CheckMenuItem(38, properties.centerText);
	_menuCoverShadow.AppendMenuItem(MF_STRING, 47, "封面显示阴影");
	_menuCoverShadow.CheckMenuItem(47, properties.showCoverShadow);
	_menuCoverShadow.AppendMenuItem(MF_STRING, 48, "设置阴影透明度 (当前:"+properties.default_CoverShadowOpacity+")");
	_menuCoverShadow.AppendTo(_menuGroupDisplay,MF_STRING, "封面阴影");

	_menuGroupDisplay.AppendTo(_menu,MF_STRING, "封面形状");

	_menuTracklist.AppendMenuItem(MF_STRING, 11, "激活音轨列表");
	...
	_additionalInfos.AppendMenuItem(custom_tag?MF_GRAYED:MF_STRING, 28, "音轨显示艺术家");
	_additionalInfos.CheckMenuItem(28, properties.showArtistName);
	_additionalInfos.AppendMenuItem(custom_tag?MF_GRAYED:MF_STRING, 56, "显示播放次数");
	_additionalInfos.CheckMenuItem(56, properties.showPlaycount);
	_additionalInfos.AppendMenuItem(custom_tag?MF_GRAYED:MF_STRING, 44, "显示编码器");
	_additionalInfos.CheckMenuItem(44, properties.showCodec);
	_additionalInfos.AppendMenuItem(custom_tag?MF_GRAYED:MF_STRING, 43, "显示比特率");
	_additionalInfos.CheckMenuItem(43, properties.showBitrate);
	_additionalInfos.AppendMenuSeparator();
	_additionalInfos.AppendMenuItem(MF_GRAYED, 0, "按顺序显示:");
	_additionalInfos.AppendMenuItem(MF_GRAYED, 0, "[艺术家名称] ([播放次数] - [编解码器] - [比特率])");


	_additionalInfos.AppendTo(_menuTracklist,MF_STRING, "音轨详情");
	
	_menuTracklist.AppendMenuSeparator();

	_menuGroupRow1.AppendMenuItem(MF_STRING, 300, "默认");
	_menuGroupRow1.AppendMenuItem(MF_STRING, 301, "自定义标题格式...");
	...
	_menuGroupRow2.AppendMenuItem(MF_STRING, 310, "默认");
	_menuGroupRow2.AppendMenuItem(MF_STRING, 311, "自定义标题格式...");
	...
	_menuGroupRow1.AppendTo(_menuGroupHeader,MF_STRING, "首行");
	_menuGroupRow2.AppendTo(_menuGroupHeader,MF_STRING, "次行");
	...
	_menuGroupHeader.AppendTo(_menuTracklist,MF_STRING, "标题");

	_menuTracklist.AppendMenuSeparator();

	_menuProgressBar.AppendMenuItem(MF_STRING, 21, "没有进度条");
	_menuProgressBar.AppendMenuItem(MF_STRING, 24, "白色进度条");
	_menuProgressBar.AppendMenuItem(MF_STRING, 23, "跟随专辑封面进度条");
	_menuProgressBar.CheckMenuRadioItem(21, 24, (!properties.drawProgressBar) ? 21 : (properties.AlbumArtProgressbar) ? 23 : 24);
	_menuProgressBar.AppendTo(_menuTracklist,MF_STRING, "进度条");

	_menuTracklist.AppendMenuSeparator();

	_menuRating.AppendMenuItem(MF_STRING, 32, "显示每首歌曲的评级");
	_menuRating.AppendMenuItem(MF_STRING, 33, "显示所选歌曲的评级");
	_menuRating.AppendMenuItem(MF_STRING, 36, "显示已评级歌曲的评级");
	_menuRating.AppendMenuItem(MF_STRING, 34, "显示已选和已评级歌曲的评级");
	_menuRating.AppendMenuItem(MF_STRING, 35, "不显示评级");
	...
	_menuRating.AppendTo(_menuTracklist,MF_STRING, "评级");

	_menuTracklist.AppendMenuSeparator();

	_menuBackground.AppendMenuItem(MF_STRING, 16, "跟随专辑封面背景 (主颜色)");
	_menuBackground.AppendMenuItem(MF_STRING, 17, "跟随专辑封面背景 (模糊)");
	_menuBackground.AppendMenuItem(MF_STRING, 18, "跟随专辑封面背景 (混合)");
	...
	_menuBackground.AppendTo(_menuTracklist,MF_STRING, "背景");

	_menuTracklist.AppendTo(_menu,MF_STRING, "音轨列表");
	...
	_menu2A.AppendMenuItem(MF_STRING, 221, "填充");
	_menu2A.CheckMenuItem(221, properties.wallpaperdisplay==0);
	_menu2A.AppendMenuItem(MF_STRING, 222, "适合");
	_menu2A.CheckMenuItem(222, properties.wallpaperdisplay==1);
	_menu2A.AppendMenuItem(MF_STRING, 223, "拉伸");
	_menu2A.CheckMenuItem(223, properties.wallpaperdisplay==2);
	_menu2A.AppendTo(_menu2,MF_STRING, "壁纸大小");

	_menu2.AppendTo(_menu,MF_STRING, "壁纸背景");

	_menu.AppendMenuSeparator();
	_menu.AppendMenuItem(MF_STRING, 9, "刷新图像缓存");
	...
	switch (true) {
		...
		case (idx == 60):
			...
			break;		
		case (idx == 61):
			customTracklistDetails("自定义第二行"
								,"<div class='titleBig'>自定义第二行</div><div class='separator'></div><br/>输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.<br/><a href=\"http://tinyurl.com/lwhay6f\" target=\"_blank\">点击这里</a> 了解关于工具栏标题格式信息.<br/>"
								,'默认 %artist% - %play_count% - %codec% - %bitrate%.'
								,'第2行标题格式脚本:'
		...
		case (idx == 48):
			try {
				new_value = utils.InputBox(window.ID, "输入所需的不透明度, 0 (完全透明) 到 255 (完全不透明) 之间.", "封面阴影不透明度", properties.default_CoverShadowOpacity, true);
		...
		case (idx == 301):
			try {
				new_TFsorting = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.\n\n留空重置为默认值.", "自定义排序", properties.groupCustomFirstRowTF, true);
				if (!(new_TFsorting == "" || typeof new_TFsorting == 'undefined')) {
		...
		case (idx == 311):
			try {
				new_TFsorting = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.\n\n留空重置为默认值.", "自定义排序", properties.groupCustomSecondRowTF, true);
				if (!(new_TFsorting == "" || typeof new_TFsorting == 'undefined')) {
...
oBrowser = function(name) {
   ...
   timers.firstPopulate = setTimeout(function(){
	...
	this.FormatTime = function(time) {
		...
		if (time > 0) {
			...
			txt_week = (totalW > 0) ? `${totalW} 星期${totalW > 1 ? 's' : ''} ` : '';
			txt_day = (totalD > 0) ? `${totalD} 天${totalD > 1 ? 's' : ''} ` : '';
			txt_hour = (totalH > 0) ? `${totalH} 小时 ` : '';
   ...
   this.get_metrics = function(gr){
            ...
            if(string_compare != temp){
				...
				if(i>0) {
					if(this.custom_groupby) {
						...
						this.groups[i-1].secondRow = (groupinfos_rows[1]!="")?groupinfos_rows[1]:this.groups[i-1].pl.Count+(this.groups[i-1].pl.Count>1?" 音轨":" 音轨");
					} else {
				...
				if(this.groups[i].album=="?") this.groups[i].album="单曲";
				if(this.groups[i].artist=="?") this.groups[i].artist="未知艺术家";
		...
		if(k==this.totalTracks) {
			...
			if(this.groups.length>0){
				...
				if(this.custom_groupby) {
					...
					this.groups[this.groups.length-1].secondRow = (groupinfos_rows[1]!="")?groupinfos_rows[1]:this.groups[this.groups.length-1].pl.Count+(this.groups[this.groups.length-1].pl.Count>1?" 音轨":" 音轨");
				} else {
    ...
    this.populate = function(call_id, force_sorting, keep_showlist, playlistIdx) {
		...
		console.log("--> GraphicBrowser 填充排序:"+this.currently_sorted+" call_id:"+call_id)
        this.get_albums();
		this.dont_sort_on_next_populate = false;
        ...
        if(repaint_main || repaint_f || !repaintforced){
            ...
            if(this.groups_draw.length == 0) { // library empty
				...
				if(this.firstInitialisation) {
					gr.GdiDrawText("正在载入...",  g_font.plus10, colors.normal_txt, px, py - 80, this.w, 35, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					gr.FillSolidRect(Math.round(this.w/2) -125, py-46, 250, 1, colors.border);
					gr.GdiDrawText("浏览媒体库", g_font.italicplus2, colors.faded_txt, px, py - 38, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
				}
				else {
					var playlistname=plman.GetPlaylistName(this.SourcePlaylistIdx);
					if(LibraryItems_counter<1){
						gr.GdiDrawText("没有找到音乐.",  g_font.plus10, colors.normal_txt, px, py - 80, this.w, 35, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.FillSolidRect(Math.round(this.w/2) -125, py-46, 250, 1, colors.border);
						gr.GdiDrawText("点击这里配置媒体库.", g_font.italicplus2, colors.faded_txt, px, py - 38, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						...
						gr.GdiDrawText("没有显示任何内容.", g_font.italicplus2, colors.faded_txt, px, py - 38, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					} else {
						gr.GdiDrawText(playlistname+" :", g_font.plus10, colors.normal_txt, px, py - 80, this.w, 35, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.FillSolidRect(Math.round(this.w/2) -125, py-46, 250, 1, colors.border);
						gr.GdiDrawText("空播放列表.", g_font.italicplus2, colors.faded_txt, px, py - 38, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					}
				}
            }
	...
	this.playGroup = function(group_id){
		...
		if(group_id > -1 && !this.avoidDlbePlay){
			...
			if(!getRightPlaylistState() || this.SourcePlaylistIdx==plman.PlayingPlaylist){
				...
				if(fb.IsPaused) fb.Stop();
				plman.FlushPlaybackQueue();
				fb.RunContextCommandWithMetadb("添加到播放队列", this.groups[this.groups_draw[group_id]].pl[0]);
				fb.Play();
			} else {
				plman.ClearPlaylist(PlaybackPlaylist);
        ...
        switch(event) {
            ...
            case "mbtn_down":
                if(this.activeIndex > -1){
					fb.RunContextCommandWithMetadb("属性",this.groups[this.groups_draw[this.activeIndex]].pl);
                }
                break;
            case "move":
				this.setActiveRow(x,y);
    ...
    this.setResizeButton = function (w,h) {
        ...
        if(typeof(this.resize_bt) == "undefined") {
            this.resize_bt = new button(this.ResizeButton_off, this.ResizeButton_hover, this.ResizeButton_hover,"resize_bt","封面大小调整");
        } else {
...
function on_mouse_rbtn_down(x, y){
    ...
    if(!g_dragA && !g_dragR && wh > 10) {
        ...
        if(check__ > -1) {
			...
			quickSearchMenu.AppendMenuItem(MF_STRING, 30,"同一艺术家");
			quickSearchMenu.AppendMenuItem(MF_STRING, 31,"同一专辑");
			quickSearchMenu.AppendMenuItem(MF_STRING, 32,"同一流派");
			quickSearchMenu.AppendMenuItem(MF_STRING, 33,"同一日期");
			...
			genrePopupMenu.AppendTo(_menu, MF_STRING, "编辑流派");
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 19, "刷新此图像");
			_menu.AppendMenuSeparator();

			sendTo.AppendTo(_menu, MF_STRING, "发送到...");
			sendTo.AppendMenuItem(MF_STRING, 5000, "新播放列表...");
			...
			if(!getRightPlaylistState() && brw.currentSorting=='' && !brw.currently_sorted  && !plman.IsAutoPlaylist(brw.SourcePlaylistIdx)) {
				_menu.AppendMenuItem(MF_STRING, 16, "移除播放列表项目");
			}
                    ...
                    if(g_showlist.columns[c]) {
                        for(var r = 0; r < g_showlist.columns[c].rows.length; r++) {
                            if(g_showlist.columns[c].rows[r].check("right", x, y)) {
								...
								var quickSearchMenu = window.CreatePopupMenu();
								quickSearchMenu.AppendMenuItem(MF_STRING, 34,"同一标题");
								quickSearchMenu.AppendMenuItem(MF_STRING, 30,"同一艺术家");
								quickSearchMenu.AppendMenuItem(MF_STRING, 31,"同一专辑");
								quickSearchMenu.AppendMenuItem(MF_STRING, 32,"同一流派");
								quickSearchMenu.AppendMenuItem(MF_STRING, 33,"同一日期");
								...
								if(!getRightPlaylistState() && brw.currentSorting=='' && !plman.IsAutoPlaylist(brw.SourcePlaylistIdx)) {
									...
									if(metadblist_selection.Count>1)
										_menu.AppendMenuItem(MF_STRING, 17, "移除播放列表项目");
									else
										_menu.AppendMenuItem(MF_STRING, 17, "移除播放列表项目");
		...
		if(!track_clicked && !album_clicked){
			...
			contextMenu.AppendTo(_menu, MF_STRING, "音轨快捷菜单");	
		...
		if(utils.IsKeyPressed(VK_SHIFT)) {
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 7, "属性 ");
			_menu.AppendMenuItem(MF_STRING, 6, "配置...");
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 5, "重载入");
		}
        idx = _menu.TrackPopupMenu(x,y);
        switch(true) {
		    case (idx == 1):
...
function on_mouse_move(x, y, m) {
	...
	if(isResizing){
		...
		if(g_dragR) {
			...
			else drag_img = createDragText("拖动", items.Count+" 音轨", 220);
			brw.external_dragging = true;
			var options = {
				show_text : false,
				use_album_art : false,
	...
	if (mask == KMask.none) {
		...
		case VK_DELETE:
			if(g_showlist.haveSelectedRows()){
				...
				if(!plman.IsAutoPlaylist(brw.getSourcePlaylist()) && metadblist_selection.Count>0) {
					...
					HtmlDialog("请确认", QuestionString, '是', '否', delete_confirmation);
				};
			}
			break;
		};
		...
		switch(mask) {
			...
			case KMask.alt:
				switch(vkey) {
				case 65: // ALT+A
					fb.RunMainMenuCommand("视图/总在最前端");
					break;
