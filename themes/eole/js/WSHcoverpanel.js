...
function setTooltipDoubleClicText(){
	switch(true){
		case (properties.dble_click_action==0):
			tooltipDoubleClicText = "暂停播放";
		break;
		case (properties.dble_click_action==1):
			tooltipDoubleClicText = "所有面板上显示";
		break;
		case (properties.dble_click_action==2):
			tooltipDoubleClicText = "打开完全/原始大小封面";
		break;
		case (properties.dble_click_action==3):
			tooltipDoubleClicText = "打开包含的文件夹";
		break;
		case (properties.dble_click_action==4):
			tooltipDoubleClicText = "激活/退出 Mini 播放器";
		break;
	}
}
...
function on_mouse_rbtn_up(x, y) {
  ...
  let now_playing_track;
	...
	if(fb.IsPlaying){
		...
		main_menu.AppendMenuItem(MF_STRING, 9, "在全部面板上显示正在播放");
		...
		main_menu.AppendMenuItem(MF_DISABLED, 0, "随机播放 :");
		main_menu.AppendMenuSeparator();
		main_menu.AppendMenuItem(MF_STRING, 3, "音轨");
		main_menu.AppendMenuItem(MF_STRING, 2, "专辑");
		main_menu.AppendMenuItem(MF_STRING, 5, "艺术家");
    main_menu.AppendMenuItem(MF_STRING, 4, "流派");
    ...
    properties.random_function === '1_artist' && (checked_item_menu = 5);
		...
		genrePopupMenu.AppendTo(main_menu, MF_STRING, "指定流派");
	}
	
	fb.IsPlaying && main_menu.AppendMenuItem(MF_STRING, 200, "属性");
