...
function CreateAutoPlaylist(pl_place, pl_name, query) {
	try {
		result = utils.InputBox(window.ID, "命名播放列表名称.", "创建智能播放列表", "新智能播放列表", true);
		if (result){
			if (result == "") {
				plman.CreateAutoPlaylist(pl_place, "新智能播放列表", query);
...
function DeletePlaylist(delete_pid) {
	...
	HtmlDialog("删除播放列表", "删除播放列表 '"+parsed_tabname+"' ?", '是', '否', delete_confirmation);
}
/*
===================================================================================================
    Objects
...
oFilterBox = function() {
    ...
    this.getImages = function() {
		...
		this.search_bt = new button(this.images.search_icon, this.images.search_icon, this.images.search_icon,"search_bt", "过滤播放列表");
	...
	this.on_init = function() {
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", "播放列表...", colors.normal_txt, 0, 0, colors.selected_bg, g_sendResponse, "brw", undefined, "g_font.plus1");
...
oBrowser = function(name) {
        ...
        if(repaint_main || !repaintforced){
			...
			if((properties.showNewPlaylistButton && this.rows.length <= 1) || (!properties.showNewPlaylistButton && this.rows.length == 0)) {
				if(g_first_populate_done){ // no playlist, manager panel is empty
						...
						gr.GdiDrawText("正在载入...", g_font.plus5, colors.normal_txt, this.x, py - 40, this.w, 36, DT_CENTER | DT_BOTTOM | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
						gr.FillSolidRect(this.x+Math.round(this.w/2-line_width/2),py, line_width, 1, colors.border);
            ...
            if(properties.showHeaderBar) {
				...
				else var boxText = this.rows.length+" 播放列表"+(this.rows.length>1?" ":"");
	...
	this.context_menu = function(x, y, id) {
        ...
        _newplaylist.AppendMenuItem(MF_STRING, 100, "播放列表");
        _newplaylist.AppendMenuItem(MF_STRING, 101, "新智能播放列表");
        _autoplaylist.AppendTo(_newplaylist, MF_STRING, "预设播放列表");
        //_autoplaylist.AppendMenuItem(MF_STRING, 200, "Media Library (full)");
        _autoplaylist.AppendMenuItem(MF_STRING, 205, "历史 (过去几周播放的音轨)");
        _autoplaylist.AppendMenuItem(MF_STRING, 206, "经常播放的音轨");
        _autoplaylist.AppendMenuItem(MF_STRING, 207, "评级最高的音轨");
        _autoplaylist.AppendMenuItem(MF_STRING, 210, "新添加的音轨");
        ...
        if(!add_mode) {
            ...
            _menu.AppendMenuItem(MF_STRING, 3, "重命名播放列表");
            _menu.AppendMenuItem(MF_STRING, 8, "删除播放列表");


            if(plman.IsAutoPlaylist(this.rows[id].idx)) {
                _menu.AppendMenuItem(MF_SEPARATOR, 0, "");
                _menu.AppendMenuItem(MF_STRING, 6, "智能播放列表属性...");
        ...
        case (idx==101):
			...
			if(CreateAutoPlaylist(total, new_label, "在此输入查询")){
				plman.MovePlaylist(total, pl_idx);
				plman.ActivePlaylist = pl_idx;
			}
        ...
        case (idx==3):
			try {
				playlistname = utils.InputBox(window.ID, "重命名播放列表: "+plman.GetPlaylistName(pl_idx), "重命名播放列表", plman.GetPlaylistName(pl_idx), true);
        ...
        case (idx==4):
            ...
            break;
        case (idx==5):
            plman.DuplicatePlaylist(pl_idx, "副本 - " + plman.GetPlaylistName(pl_idx));
    ...
    this.settings_context_menu = function(x, y) {
		...
		_rowHeight.AppendMenuItem(MF_STRING, 1001, "减少");
		...
		//_menu2.AppendMenuItem((!properties.showwallpaper ? MF_GRAYED | MF_DISABLED : MF_STRING), 210, "默认");
        ...
        if(brw.inputboxID >= 0) {
            ...
            if (mask == KMask.none) {
                ...
                case VK_F2:
					...
					try {
						playlistname = utils.InputBox(window.ID, "重命名播放列表: "+plman.GetPlaylistName(brw.rows[rowId].idx), "重命名播放列表", plman.GetPlaylistName(brw.rows[rowId].idx), true);
                ...
                case VK_RETURN:
                    if(brw.rowsCount > 0) {
						...
						} catch(e){plman.ActivePlaylist = brw.rowsCount - 1;console.log("激活错误")}
                        cPlaylistManager.playlist_switch_pending = true;
                    };
                    break;
				case VK_END:
...
function on_playlist_items_added(playlist_idx) {
	try{
		...
		console.log('on_playlist_items_added 失败');
	}
}
...
function on_playlist_items_removed(playlist_idx, new_count) {
	try{
		...
		console.log('on_playlist_items_removed 失败');
	}
}
...
function on_drag_drop(action, x, y, mask) {
    if(y > brw.y) {
        ...
        if(brw.activeRow > -1) {
			...
			if(g_dragndrop_targetPlaylistId > -1 && brw.rows[g_dragndrop_targetPlaylistId].isAutoPlaylist) {
				action.Effect = 0;
				HtmlMsg("错误", "'"+plman.GetPlaylistName(g_dragndrop_targetPlaylistId)+"' 自动生成的播放列表 (智能播放列表).\n不能手动添加音轨到智能播放列表中", "确定");
            } else if(g_dragndrop_targetPlaylistId > -1 && brw.rows[g_dragndrop_targetPlaylistId].idx>-1) {
                action.Effect = 1;
				...
				plman.CreatePlaylist(total_pl, "拖放项目");
				action.Effect = 1;
		...
		if(rename_playlist){
			try {
				playlistname = utils.InputBox(window.ID, "命名播放列表名称 ?", "重命名播放列表", plman.GetPlaylistName(total_pl), true);
