var properties_big = {
    ...
    defaultPlaylistItemAction: window.GetProperty("BIG:defaultPlaylistItemAction", "播放"), //"添加到播放队列"
...
var properties_mini = {
    ...
    defaultPlaylistItemAction: window.GetProperty("MINI:defaultPlaylistItemAction", "播放"), //"添加到播放队列"
...
var properties_common = {
    ...
    tf_groupkey: fb.TitleFormat("$if2(%album%$ifgreater(%totaldiscs%,1,[' - 光碟 '%discnumber%],),$if(%length%,'?',%path%)) ^^ $if2(%album artist%,$if(%length%,'未知艺术家',%title%)) ^^ %discnumber% ## $if2(%artist%,$if(%length%,'未知艺术家',%path%)) ^^ %title% ^^ [%genre%] ^^ [%date%]"),
    tf_track: fb.TitleFormat("%tracknumber% ^^ $if(%length%,%length%,广播) ^^ $if2(" + (globalProperties.use_ratings_file_tags ? "$meta(rating)" : "%rating%") + ",0) ^^ %mood% ^^ %play_count% ^^ %bitrate% ^^ %codec%"),
    tf_path: fb.TitleFormat("$directory_path(%path%)\\"),
    tf_time_remaining: fb.TitleFormat("$if(%length%,-%playback_time_remaining%,'广播')"),
    tf_elapsed_seconds: fb.TitleFormat("$if(%length%,%playback_time_seconds%,'广播')"),
    tf_total_seconds: fb.TitleFormat("$if2(%length_seconds%,'广播')"),
    ...
    this.draw = function(gr) {
        ...
        if(this.offset > 0) {
           ...
           // gr.FillSolidRect(cx + bg_margin_left, this.y + cPlaylistManager.topbarHeight - 2, this.w - bg_margin_left*2, 1, colors.pm_bg4);
            ...
            if(cPlaylistManager.blink_counter > -1) {
                if(cPlaylistManager.blink_row == 0) {
					gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 34, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                    if(cPlaylistManager.blink_counter <= 6 && Math.floor(cPlaylistManager.blink_counter / 2) == Math.ceil(cPlaylistManager.blink_counter / 2)) {
                    ...
                    gr.GdiDrawText("发送到 ..." , g_font.italicplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 34, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                };
            } else {
                if(this.activeRow == 0) {
                    gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 34, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
...
oFilterBox = function() {
    ...
    this.getImages = function() {
		...
		this.search_bt = new button(this.images.search_icon, this.images.search_icon, this.images.search_icon,"search_bt","音轨过滤");
        ...
        this.reset_bt = new button(this.images.resetIcon_off, this.images.resetIcon_ov, this.images.resetIcon_dn,"reset-bt","重置过滤");
	};
	this.getImages();

	this.on_init = function() {
		this.inputbox = new oInputbox(cFilterBox.w, cFilterBox.h, "", "过滤", colors.normal_txt, 0, 0, colors.selected_bg, g_sendResponse, "brw", undefined, "g_font.normal");
	...
	this.set_default_text = function() {
		if(properties.lockOnNowPlaying) this.inputbox.empty_text = "播放音轨...";
		else this.inputbox.empty_text = "激活播放列表...";
	}
...
oBrowser = function(name) {
        ...
        if(repaint_main || !repaintforced){
            ...
            if(this.rows.length > 0) {
                    ...
                    case 0: // track row
                            ...
                            if(ay >= (0 - ah) && ay < this.y + this.h) {
                                ...
                                if(properties.showArtistAlways || !properties.showGroupHeaders || arr_e[0].toLowerCase() != arr_groupinfo[1].toLowerCase() || properties.doubleRowText) {
                                    var track_artist_part = ((properties.doubleRowText || track_time_part == "广播")?"":" - ")+arr_e[0]+add_infos;
                                } else {
                                ...
                                if(properties.doubleRowText) {
                                    ...
                                    if(this.nowplaying && this.rows[i].playlistTrackId_original == this.nowplaying.PlaylistItemIndex){ // now playing track
										...
										if(properties.drawProgressBar && (cNowPlaying.flashescounter<5 || !cNowPlaying.flashEnable)){
											...
											if(track_time_part == "广播" || elapsed_seconds == "广播") var current_size = properties.track_gradient_size+total_size-1;
                                    ...
                                    if(this.nowplaying && this.rows[i].playlistTrackId_original == this.nowplaying.PlaylistItemIndex) { // now playing track
										...
										if(properties.drawProgressBar && (cNowPlaying.flashescounter<5 || !cNowPlaying.flashEnable)){
											...
											var total_seconds =  g_total_seconds;
											var ratio = elapsed_seconds/total_seconds;
											if(track_time_part == "广播" || elapsed_seconds == "广播") var current_size =  progress_start + total_size;
                ...
                if(cList.search_string.length > 0) {
					...
					gr.GdiDrawText("正在载入...", g_font.plus5, colors.normal_txt, this.x, py - 40, this.w, 35, DT_CENTER | DT_BOTTOM | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					gr.FillSolidRect(this.x+this.w/2-75,py, 150, 1, colors.border);
					gr.GdiDrawText("查看播放列表", g_font.italicplus1, colors.faded_txt, this.x, py + 6, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
            };
            ...
            if(properties.showHeaderBar) {
                var boxText = (this.totaltracks > 1 ? this.totaltracks+" 项目" : this.totaltracks+" 项目");
                        ...
                        case (rowType > 0 && rowType < 99):     // ----------------> group header row
							...
							if(g_focus_id!=playlistTrackId || this.sentAlbumId!=groupId) {
								...
								notify_array = {
									...
									cover_img:null,
									totalTracks:this.groups[groupId].count+(this.groups[groupId].count > 1 ? " 音轨" : " track"),
	...
	this.context_menu = function(x, y, id, row_id, context_items) {
		...
		if(!plman.IsAutoPlaylist(plman.ActivePlaylist)){
			...
			SortMenu.AppendTo(_menu, MF_STRING, "排序按");

			SortMenu.AppendMenuItem(MF_STRING, 1036, "艺术家/专辑/音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 1037, "专辑/音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 1043, "音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 1044, "文件路径");
			SortMenu.AppendMenuItem(MF_STRING, 1045, "标题");
			SortMenu.AppendMenuItem(MF_STRING, 1040, "日期");
			SortMenu.AppendMenuItem(MF_STRING, 1041, "最短到最长");
			SortMenu.AppendMenuItem(MF_STRING, 1042, "最长到最短");
			SortMenu.AppendMenuItem(MF_STRING, 1046, "评级");			
			SortMenu.AppendMenuItem(MF_STRING, 1047, "自定义标题格式...");			
			SortMenu.AppendMenuSeparator();
			SortMenu.AppendMenuItem(MF_STRING, 1039, "随机");
		}

		var quickSearchMenu = window.CreatePopupMenu();
		quickSearchMenu.AppendMenuItem(MF_STRING, 1029,"同一标题");
		quickSearchMenu.AppendMenuItem(MF_STRING, 1030,"同一艺术家");
		quickSearchMenu.AppendMenuItem(MF_STRING, 1031,"同一专辑");
		quickSearchMenu.AppendMenuItem(MF_STRING, 1032,"同一流派");
		quickSearchMenu.AppendMenuItem(MF_STRING, 1033,"同一日期");
		quickSearchMenu.AppendTo(_menu, MF_STRING, "快速搜索...");

		_menu.AppendMenuSeparator();
		_menu.AppendMenuItem(MF_STRING, 1034, "全部选择");
		if(this.metadblist_selection.Count > 0) _menu.AppendMenuItem(plman.IsAutoPlaylist(g_active_playlist)?MF_DISABLED|MF_GRAYED:MF_STRING, 1020, "移除所选");
    ...
    this.settings_context_menu = function(x, y) {
			...
			lockOnMenu.AppendTo(_menu, MF_STRING, "显示播放列表");
			lockOnMenu.AppendMenuItem(MF_STRING, 3300, "正在播放列表");
			lockOnMenu.AppendMenuItem(MF_STRING, 3299, "激活播放列表");
			lockOnMenu.AppendMenuSeparator();
			lockOnMenu.AppendMenuItem(MF_STRING, 3298, "自动切换到正在播放或活动播放列表");
			lockOnMenu.CheckMenuItem(3298, properties.enableAutoSwitchPlaylistMode);
			lockOnMenu.AppendMenuItem(MF_STRING, 3297, "正在播放列表: 没有播放时显示活动列表");
            ...
            _menu.AppendMenuItem(MF_STRING, 899, "关注正在播放");
			_menu.CheckMenuItem(899, properties.FollowNowPlaying);
            _menu.AppendMenuItem(((fb.IsPlaying && !(!properties.enableAutoSwitchPlaylistMode && properties.lockOnPlaylistNamed!="")) ? MF_STRING : MF_GRAYED | MF_DISABLED), 900, "显示正在播放");
            _menu.AppendMenuItem(MF_STRING, 901, "启用拖放到播放列表");
			_menu.CheckMenuItem(901, properties.DropInplaylist);

            _menu.AppendMenuSeparator();
            _menu.AppendMenuItem(MF_STRING, 112, "圆形封面");
			...
			_rowStyle.AppendMenuItem(MF_STRING, 911, "单行");
			_rowStyle.AppendMenuItem(MF_STRING, 912, "双行");
			_rowStyle.AppendMenuItem(MF_STRING, 913, "双行封面");
			_rowStyle.CheckMenuRadioItem(911, 913, (!properties.doubleRowText) ? 911 : (!properties.doubleRowShowCover) ? 912 : 913);
			_rowStyle.AppendTo(_menu,MF_STRING, "行样式");

            _menu3.AppendMenuItem(MF_STRING, 300, "启用");
            _menu3.CheckMenuItem(300, properties.showGroupHeaders);
            _menu3.AppendMenuItem((properties.showGroupHeaders ? MF_STRING : MF_GRAYED | MF_DISABLED), 310, "自动折叠");
            _menu3.CheckMenuItem(310, properties.autocollapse);

            _menu3.AppendMenuSeparator();
            _menu3.AppendMenuItem((properties.showGroupHeaders && !properties.autocollapse ? MF_STRING : MF_GRAYED | MF_DISABLED), 320, "全部折叠");
            _menu3.AppendMenuItem((properties.showGroupHeaders && !properties.autocollapse ? MF_STRING : MF_GRAYED | MF_DISABLED), 330, "全部展开");

            _menu3.AppendTo(_menu,MF_STRING, "分组标题");

            _menu.AppendMenuItem(MF_STRING, 910, "搜索栏");
            _menu.CheckMenuItem(910, properties.showHeaderBar);

            _menu.AppendMenuSeparator();

			var _menu_title = window.CreatePopupMenu();
            _menu_title.AppendMenuItem(MF_STRING, 905, "长音轨标题工具条显示提示");
			_menu_title.CheckMenuItem(905, properties.showToolTip);
            _menu_title.AppendMenuItem(MF_STRING, 906, "显示播放次数");
			_menu_title.CheckMenuItem(906, properties.showPlaycount);
            _menu_title.AppendMenuItem(MF_STRING, 908, "显示编码器");
			_menu_title.CheckMenuItem(908, properties.showCodec);
            _menu_title.AppendMenuItem(MF_STRING, 907, "显示比特率");
			_menu_title.CheckMenuItem(907, properties.showBitrate);
            _menu_title.AppendMenuItem((!properties.doubleRowText ? (!properties.showGroupHeaders ? MF_GRAYED | MF_DISABLED : MF_STRING) : MF_GRAYED | MF_DISABLED), 111, "总是将艺术家应用到标题");
            _menu_title.CheckMenuItem(111, properties.showArtistAlways);
			_menu_title.AppendTo(_menu,MF_STRING, "音轨信息");

			_menu1.AppendMenuItem(MF_STRING, 914, "没有进度条");
			_menu1.AppendMenuItem(MF_STRING, 916, "白色进度条");
			_menu1.AppendMenuItem(MF_STRING, 915, "跟随专辑封面进度条");
			_menu1.CheckMenuRadioItem(914, 916, (!properties.drawProgressBar) ? 914 : (properties.AlbumArtProgressbar) ? 915 : 916);
            _menu1.AppendTo(_menu,MF_STRING, "播放标题下的进度条");

			_menuRating.AppendMenuItem(MF_STRING, 113, "显示每首歌曲的评级");
			_menuRating.AppendMenuItem(MF_STRING, 114, "显示所选歌曲的评级");
			_menuRating.AppendMenuItem(MF_STRING, 117, "显示已评级歌曲的评级");
			_menuRating.AppendMenuItem(MF_STRING, 115, "显示已选和已评级歌曲的评级");
			_menuRating.AppendMenuItem(MF_STRING, 116, "不显示评级");
			...
			_menuRating.AppendTo(_menu,MF_STRING, "显示评级");

			if (layout_state === 0){
				var _panelWidth = window.CreatePopupMenu();
				_panelWidth.AppendMenuItem(MF_STRING, 2030, "增加宽度");
				_panelWidth.AppendMenuItem(MF_STRING, 2031, "减少宽度");
				_panelWidth.AppendMenuItem(MF_STRING, 2033, "自定义宽度...");
			...
			if (layout_state === 0 && main_panel_state === 1) {
				_menu.AppendMenuSeparator();
				_menu.AppendMenuItem(MF_STRING, 993, "隐藏播放列表");
			}
            ...
            if (idx > 3300) {
                ...
                switch(idx) {
                    ...
                    case 2032:
                        ...
                        break;
                    case 2033:
                        rightplaylist_width.userInputValue("以像素为单位输入所需宽度.\n默认宽度为 270px.\n最小宽度: 100px. 最大宽度: 900px", "自定义左侧菜单宽度");
                        break;
                    case 3297:
                        ...
                        if(clipboard.selection) {
                            if(clipboard.selection.Count > 0) {
                                try {
                                    ...
                                    console.log("WSH 面板错误: 剪贴板无法粘贴, 剪贴板内容无效.");
                                };
                            };
                        };
                    };
...
function on_playback_dynamic_info_track() {
	...
	if(g_time_remaining=="广播" || g_total_seconds=="广播") {
...
function on_playback_new_track(metadb) {
	...
	if(window.IsVisible){
		...
		g_total_seconds =  properties.tf_total_seconds.Eval(true);
		if(g_total_seconds!="广播") g_time_remaining = "-"+g_total_seconds.toHHMMSS();
		else {
			g_time_remaining = "广播";
...
function on_playback_time(time) {
	...
	if(window.IsVisible && plman.PlayingPlaylist==g_active_playlist){
		if(g_total_seconds>0 && g_total_seconds!="广播"){
...
function on_metadb_changed(metadbs, fromhook) {
	...
	try {
		...
		console.log("错误:on_metadb_changed, WSHsmoothplaylist try/catch");
...
function on_drag_leave() {
    ...
    if(g_dragndrop_drop_forbidden) {
		//fb.ShowPopupMessage("The current playlist is an autoplaylist: you can't reorder tracks in an autoplaylist.", "错误");
        g_dragndrop_drop_forbidden = false;
        window.Repaint();
    };
};
...
function on_drag_over(action, x, y, mask) {
    ...
    brw.on_mouse("drag_over", x, y);
	try{
		//if(brw.drag_tracks) action.Text = "移动";
		//else action.Text = "插入";
	} catch(e){}
...
function on_drag_drop(action, x, y, mask) {
    ...
    if(g_active_playlist >=0 && plman.IsAutoPlaylist(g_active_playlist)) {
		if(g_dragndrop_drop_forbidden) fb.ShowPopupMessage("当前播放列表是一个智能播放列表: 您不能手动添加一个音轨到智能播放列表中.", "错误");
        g_dragndrop_drop_forbidden = false;
		action.Effect = 0;
