...
var properties = {
	...
	default_firstRow: fb.TitleFormat("$if2(%discnumber%.,)$num(%tracknumber%,1) ^^ %title% ^^ $if3($meta(artist),$meta(album artist),$meta(composer),$meta(performer))"),
...
var time_t = fb.TitleFormat("$if(%length_seconds%,%length_seconds%,'广播')");
var time_r = fb.TitleFormat("$if(%length%,-%playback_time_remaining%,%playback_time%)");
var tf_elapsed_seconds = fb.TitleFormat("$if2(%playback_time_seconds%,'广播')");
...
function build_buttons(){
	...
	buttons = {
		Prev: new SimpleButton(button_left_m, button_top_m, 41, 41, "上一首", "上一首" ,false, function () {
			fb_play_from_playing(-1) //fb.Prev();
			g_tooltip.Deactivate();
		},false,prev_img,prev_img_hover),
		Pause: new SimpleButton(button_sep_value+button_left_m, button_top_m, 41, 41, "播放", "播放/暂停",false, function () {
			if(!fb.IsPlaying) fb.Play();
			else fb.Pause();
			g_tooltip.Deactivate();
		},false,play_img,play_img_hover),
		Next: new SimpleButton(button_sep_value*2+button_left_m, button_top_m, 41, 41, "下一首", "下一首",false, function () {
			fb_play_from_playing(1) //fb.Next();
			g_tooltip.Deactivate();
		},false,next_img,next_img_hover),
		Stop: new SimpleButton(button_sep_value*3+button_left_m, button_top_m, 41, 41, "停止", "停止",false, function () {
			fb.Stop();
			g_tooltip.Deactivate();
		},false,stop_img,stop_img_hover)
	}
	buttons_mini = {
		/*Volume: new SimpleButton(mini_btns.button_left_m, mini_btns.button_top_m, 15, 15, "下一首", "",false, function () {
			showVolumeSlider();
		},false,mini_volume_img,mini_volume_img_hover),	*/
		Prev: new SimpleButton(-75-mini_btns.button_width-2, mini_btns.button_top_m, 21, 21, "上一首", "上一首",false, function () {
			fb_play_from_playing(-1) //fb.Prev();
			g_tooltip.Deactivate();
		},false,mini_prev_img,mini_prev_img_hover),
		/*Pause: new SimpleButton(mini_btns.button_width*2+mini_btns.button_left_m, mini_btns.button_top_m, 15, 15, "mini播放", "",false, function () {
			if(!fb.IsPlaying) fb.Play();
			else fb.Pause();
		},false,mini_play_img,mini_play_img_hover),*/
		Next: new SimpleButton(-75, mini_btns.button_top_m, 21, 21, "下一首", "下一首",false, function () {
			fb_play_from_playing(1) //fb.Next();
			g_tooltip.Deactivate();
	...
	displayed_button = 0;
	buttons_right = {
		More: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "更多", "其它控制...", function () {
				moreMenu(window.Width, 15);
				g_tooltip.Deactivate();
		},false,false,more_img,more_img_hover),
		Scheduler: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "电源和休眠", "停止/关闭电源和休眠...", function () {
		...
		Rating: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "评级", "当前歌曲评级", false, function () {
			g_tooltip.Deactivate();
			if(fb.IsPlaying) {
		...
		Equalizer: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "均衡器", "显示均衡器",false, function () {
			g_tooltip.Deactivate();
			fb.RunMainMenuCommand("视图/DSP/均衡器");
		},false,equalizer_img,equalizer_img_hover),
		Device: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "设备", "输出设备 & DSP 预设", function () {
			g_tooltip.Deactivate();
			menuOutputAndDSP(window.Width-button_right_m-(button_width+button_padding)*1-130, 15);
		},false,false,device_img,device_img_hover),		
		Open: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "打开", "打开文件...",false, function () {
			g_tooltip.Deactivate();
			fb.AddFiles()
		},false,open_img,open_img_hover),
		playRandom: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "随机播放", "随机选择音轨播放...", function () {
			g_tooltip.Deactivate();
			randomPlayMenu(window.Width-button_right_m-(button_width+button_padding)*1-130, 15);
		},false,false,random_img,random_img_hover),
		Shuffle: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "乱序", "乱序 开/关"+"\n"+"(双击随机播放列表)",false, function () {
			g_tooltip.Deactivate();
		...
		Repeat: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "重复", "重复模式",false, function () {
			g_tooltip.Deactivate();
			...
			g_tooltip.ActivateDelay(get_repeat_tooltip(), g_cursor.x+10, g_cursor.y+20, globalProperties.tooltip_button_delay, 1200, false, '重复');
		},false,repeat_img,repeat_img_hover),
		Volume: new SimpleButton(-button_right_m-(button_width+button_padding)*(displayed_button++), buttons_right_top_m, button_width, 32, "音量", "音量 (点击静音)",false, function () {
			g_tooltip.Deactivate();
			fb.VolumeMute();
...
function showVolumeSlider(new_state){
	...
	if(toggle){
		for (var i in buttons_right) {
			if(buttons_right[i].text!="音量") buttons_right[i].Togglehide();
		}
		for (var i in buttons_mini) {
			if(buttons_mini[i].text!="音量") buttons_mini[i].Togglehide();
		}
		VolumeSliderActive=!VolumeSliderActive;
	} else {
		for (var i in buttons_right) {
			if(buttons_right[i].text!="音量") buttons_right[i].hide = new_state;
		}
		for (var i in buttons_mini) {
			if(buttons_mini[i].text!="音量") buttons_mini[i].hide = new_state;
		}
...
function on_mouse_lbtn_down(x,y,m){
	...
	//Time Display
    if (is_hover_time_elapsed(x,y) && TimeTotalSeconds!="广播") {
...
function on_mouse_move(x,y,m){
	...
	if(is_hover_progress(x,y) && TimeTotalSeconds!="广播"){
...
function draw_menu() {
    ...
    child7.AppendTo(basemenu, MF_STRING, "正在播放");

	basemenu.AppendMenuSeparator();
	var _schedulerMenu = window.CreatePopupMenu();
	_schedulerMenu.AppendMenuItem(MF_STRING, 3018, "无操作");
	_schedulerMenu.AppendMenuSeparator();
	_schedulerMenu.AppendMenuItem(MF_STRING, 3019, "当前结束后停止");
	_schedulerMenu.AppendMenuItem(MF_STRING, 3020, "当前结束后休眠");
	_schedulerMenu.AppendMenuItem(MF_STRING, 3021, "当前结束后关机");
	_schedulerMenu.AppendMenuSeparator();
	_schedulerMenu.AppendMenuItem(MF_STRING, 3022, "播放列表结束后休眠");
	_schedulerMenu.AppendMenuItem(MF_STRING, 3023, "播放列表结束后关机");
	checked_item=0;
	switch (true) {
		case (scheduler.hibernate_after_current):
			checked_item=3020
	...
	_schedulerMenu.AppendTo(basemenu, MF_STRING, "电源和休眠");
	...
	if(main_panel_state.isEqual(0)){
		basemenu.AppendMenuSeparator();
		library_menu.AppendTo(basemenu,MF_STRING, "当前面板设置");
		library_menu.AppendMenuItem(MF_STRING, 4000, "启用深色主题");
		...
		basemenu.AppendMenuSeparator();
		playlists_menu.AppendTo(basemenu,MF_STRING, "当前面板设置");
		playlists_menu.AppendMenuItem(MF_STRING, 4001, "启用深色主题");
		...
		basemenu.AppendMenuSeparator();
		bio_menu.AppendTo(basemenu,MF_STRING, "当前面板设置");
		bio_menu.AppendMenuItem(MF_STRING, 4002, "启用深色主题");
		...
		basemenu.AppendMenuSeparator();
		visu_menu.AppendTo(basemenu,MF_STRING, "当前面板设置");
		visu_menu.AppendMenuItem(MF_STRING, 4003, "启用深色主题");
...
function randomPlayMenu(x, y){
		...
		genrePopupMenu.AppendTo(_menu, MF_STRING, "指定流派");
        ...
        switch(true) {
            case (idx == 2):
                ...
                window.SetProperty("Random function", properties.random_function);
				window.NotifyOthers("随机播放", properties.random_function);
                break;
            case (idx == 3):
...
function menuOutputAndDSP(x, y){
	...
	menu.AppendMenuItem(MF_GRAYED, 0, "输出设备:");
	...
	menu.AppendMenuSeparator();
	menu.AppendMenuItem(MF_GRAYED, 0, "DSP 预设:");
	menu.AppendMenuSeparator();	
	if(dsps.length>0){
		var active = -1;
		...
		menu.AppendMenuItem(MF_GRAYED, 0, "没有定义任何 DSP 预设.");
		menu.AppendMenuItem(MF_GRAYED, 0, "Foobar > 文件 > 参数选项 > 播放 > DSP 管理器");
	}
...
function moreMenu(x, y){
		...
		if(fb.IsPlaying) {
			_moreMenu.AppendMenuItem(MF_STRING, 1, "显示正在播放");
			if(!properties.displayRating) {
				var _ratingMenu = window.CreatePopupMenu();
				_ratingMenu.AppendMenuItem(MF_STRING, 2001, "0/未设置");
				...
				_ratingMenu.AppendTo(_moreMenu, MF_STRING, "评级");
			}
		...
		if (fb.IsPlaying && !fb.IsPaused) _controlsMenu.AppendMenuItem(MF_STRING, 3001, "暂停");
		...
		_controlsMenu.AppendMenuItem(MF_STRING, 3005, "随机");
		_controlsMenu.AppendMenuSeparator();
			var _playbackOrder = window.CreatePopupMenu();
			_playbackOrder.AppendMenuItem(MF_STRING, 3007, "默认");
			_playbackOrder.AppendMenuItem(MF_STRING, 3008, "重复 (播放列表)");
			_playbackOrder.AppendMenuItem(MF_STRING, 3009, "重复 (音轨)");
			_playbackOrder.AppendMenuItem(MF_STRING, 3010, "随机");
			_playbackOrder.AppendMenuItem(MF_STRING, 3011, "乱序 (音轨)");
			_playbackOrder.AppendMenuItem(MF_STRING, 3012, "乱序 (专辑)");
			_playbackOrder.AppendMenuItem(MF_STRING, 3013, "乱序 (文件夹)");
			...
			_playbackOrder.AppendMenuItem(MF_STRING, 3024, "随机播放列表");
			_playbackOrder.AppendTo(_controlsMenu, MF_STRING, "顺序");
		_controlsMenu.AppendMenuSeparator();
		_controlsMenu.AppendMenuItem(MF_STRING, 3006, "刷新播放队列");
		...
		if(!properties.displayOpen) {
			...
			_openMenu.AppendMenuItem(MF_STRING, 3025, "打开文件...");
			_openMenu.AppendMenuItem(MF_STRING, 3026, "打开音频 CD...");
			_openMenu.AppendMenuSeparator();
			_openMenu.AppendMenuItem(MF_STRING, 3027, "添加文件...");
			_openMenu.AppendMenuItem(MF_STRING, 3028, "添加目录...");
			_openMenu.AppendMenuItem(MF_STRING, 3029, "添加位置...");
			_openMenu.AppendTo(_moreMenu, MF_STRING, "打开");
		}

		if(!properties.displayPlayRandom) {
			var _playRandom = window.CreatePopupMenu();
			var checked_item_menu=503;
			_playRandom.AppendMenuItem(MF_STRING, 503, "音轨");
				if(properties.random_function=='200_tracks') checked_item_menu=503;
			_playRandom.AppendMenuItem(MF_STRING, 502, "专辑");
				if(properties.random_function=='20_albums') checked_item_menu=502;
			_playRandom.AppendMenuItem(MF_STRING, 505, "艺术家");
				if(properties.random_function=='1_artist') checked_item_menu=505;

			var genreValue=parseInt(properties.random_function);
				_playRandom.AppendMenuItem(MF_STRING, 504, "流派");
			...
			var genrePopupMenu = window.CreatePopupMenu();
			createGenrePopupMenu(false, -1, genrePopupMenu);
			genrePopupMenu.AppendTo(_playRandom, MF_STRING, "指定流派");

			_playRandom.AppendTo(_moreMenu, MF_STRING, "随机播放");
		}
		...
		if(!properties.displayScheduler) {
			...
			_schedulerMenu.CheckMenuRadioItem(3018, 3023, checked_item);
			_schedulerMenu.AppendTo(_moreMenu, MF_STRING, "电源和休眠");
		}

		if(!properties.displayEqualizer)
			_moreMenu.AppendMenuItem(MF_STRING, 3017, "均衡器");

		var _menu_button = window.CreatePopupMenu();
		_moreMenu.AppendMenuSeparator();
		_menu_button.AppendMenuItem(MF_STRING, 4016, "停止");
		_menu_button.CheckMenuItem(4016, properties.displayStop);
		_menu_button.AppendMenuItem(MF_STRING, 4022, "重复");
		_menu_button.CheckMenuItem(4022, properties.displayRepeat);
		_menu_button.AppendMenuItem(MF_STRING, 4021, "乱序");
		_menu_button.CheckMenuItem(4021, properties.displayShuffle);
		_menu_button.AppendMenuItem(MF_STRING, 4023, "打开");
		_menu_button.CheckMenuItem(4023, properties.displayOpen);
		_menu_button.AppendMenuItem(MF_STRING, 4020, "随机播放");
		_menu_button.CheckMenuItem(4020, properties.displayPlayRandom);
		_menu_button.AppendMenuItem(MF_STRING, 4017, "均衡器");
		_menu_button.CheckMenuItem(4017, properties.displayEqualizer);
		_menu_button.AppendMenuItem(MF_STRING, 4024, "输出设备 & DSP 预设");
		_menu_button.CheckMenuItem(4024, properties.displayDevice);		
		_menu_button.AppendMenuItem(MF_STRING, 4018, "评级");
		_menu_button.CheckMenuItem(4018, properties.displayRating);
		_menu_button.AppendMenuItem(MF_STRING, 4019, "电源和休眠");
		_menu_button.CheckMenuItem(4019, properties.displayScheduler);

		_menu_button.AppendTo(_moreMenu,MF_STRING, "按钮显示");
        ...
        switch(true) {
            ...
            case (idx == 3025):
                ...
                break;
            case (idx == 3026):
				fb.RunMainMenuCommand("文件/打开音频 CD...");
                break;
            case (idx == 3027):
				fb.RunMainMenuCommand("文件/添加文件...");
                break;
            case (idx == 3028):
				fb.RunMainMenuCommand("文件/添加目录...");
                break;
            case (idx == 3029):
...
function schedulerMenu(x, y){
		...
		var _schedulerMenu = window.CreatePopupMenu();
		_schedulerMenu.AppendMenuItem(MF_DISABLED, 0, "电源和休眠 :");
		_schedulerMenu.AppendMenuSeparator();
		_schedulerMenu.AppendMenuItem(MF_STRING, 3018, "无操作");
		_schedulerMenu.AppendMenuSeparator();
		_schedulerMenu.AppendMenuItem(MF_STRING, 3019, "当前结束后停止");
...
function draw_rating_menu(x,y){
        var _menu = window.CreatePopupMenu();
        var idx;
		_menu.AppendMenuItem(MF_DISABLED, 0, "评级:");
		_menu.AppendMenuSeparator();

		_menu.AppendMenuItem(MF_STRING, 1, "0/未设置");
...
function draw_controls_menu(x,y){
		...
		if (fb.IsPlaying && !fb.IsPaused) _menu.AppendMenuItem(MF_STRING, 3001, "暂停");
		else _menu.AppendMenuItem(MF_STRING, 3001, "播放");
		_menu.AppendMenuItem(MF_STRING, 3003, "上一首");
		_menu.AppendMenuItem(MF_STRING, 3004, "下一首");
		_menu.AppendMenuItem(MF_STRING, 3005, "随机");
		_menu.AppendMenuSeparator();

		_menu.AppendMenuItem(MF_STRING, 3024, "随机播放列表");
		_menu.AppendMenuSeparator();
		...
		_playbackOrder.AppendTo(_menu, MF_STRING, "播放顺序");
		_menu.AppendMenuSeparator();
		...
		_schedulerMenu.CheckMenuRadioItem(3018, 3023, checked_item);
		_schedulerMenu.AppendTo(_menu, MF_STRING, "电源和休眠");

		if(fb.IsPlaying){
			_menu.AppendMenuSeparator();
			var _quickSearchMenu = window.CreatePopupMenu();
			_quickSearchMenu.AppendMenuItem(MF_STRING, 34,"同一标题");
			_quickSearchMenu.AppendMenuItem(MF_STRING, 30,"同一艺术家");
			_quickSearchMenu.AppendMenuItem(MF_STRING, 31,"同一专辑");
			_quickSearchMenu.AppendMenuItem(MF_STRING, 32,"同一流派");
			_quickSearchMenu.AppendMenuItem(MF_STRING, 33,"同一日期");
			_quickSearchMenu.AppendTo(_menu, MF_STRING, "快速搜索...");			
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 2, "属性");
		}
        ...
        switch(true) {
			case (idx == 1):
                ...
                break;
			case (idx == 2):
				fb.RunContextCommandWithMetadb("属性", fb.GetNowPlaying());
			break;		
		case (idx == 30):
			if(!main_panel_state.isEqual(0) && !main_panel_state.isEqual(1)) {
				main_panel_state.setValue(0)
...
function draw_settings_menu(x,y){
		...
		_menu.AppendMenuItem(MF_STRING, 3027, "大控制栏");
		_menu.CheckMenuItem(3027,!mini_controlbar.isActive());		
		var custom_rows = properties.custom_firstRow!=""||properties.custom_secondRow!="";
		_menu_track_infos.AppendMenuItem(MF_STRING, 3015, properties.showTrackInfo?"隐藏":"显示");
		_menu_track_infos.AppendMenuItem(properties.showTrackInfo?MF_STRING:MF_GRAYED, 3040, "2 行音轨信息");
        ...
        _menu_track_infos.AppendMenuItem(properties.showTrackInfo&&!custom_rows?MF_STRING:MF_GRAYED, 9001, "显示音轨标题前缀 (碟片编号.音轨号)");
		_menu_track_infos.CheckMenuItem(9001, properties.showTrackPrefix);
        _menu_track_infos.AppendMenuItem(properties.showTrackInfo?MF_STRING:MF_GRAYED, 3031, "自定义...");
		_menu_track_infos.CheckMenuItem(3031, custom_rows);
        if(custom_rows) _menu_track_infos.AppendMenuItem(properties.showTrackInfo?MF_STRING:MF_GRAYED, 3032, "重置");		
		_menu_track_infos.AppendTo(_menu,MF_STRING, "音轨详情");
		
		_menu.AppendMenuItem(MF_STRING, 3030, "总是显示音量条");
		_menu.CheckMenuItem(3030, properties.volumeA1waysVisible);		
		_menu.AppendMenuItem(MF_STRING, 3016, "显示正在播放封面");
		_menu.CheckMenuItem(3016, (layout_state.isEqual(0)?coverpanel_state_big.isActive():coverpanel_state_mini.isActive()));

		_menu.AppendMenuSeparator();
		_menu3.AppendMenuItem(MF_STRING, 200, "启用");
		_menu3.CheckMenuItem(200, properties.showwallpaper);
		_menu3.AppendMenuItem(MF_STRING, 220, "模糊");
		_menu3.CheckMenuItem(220, properties.wallpaperblurred);

		_menu3A.AppendMenuItem(MF_STRING, 221, "填充");
		_menu3A.CheckMenuItem(221, properties.wallpaperdisplay==0);
		_menu3A.AppendMenuItem(MF_STRING, 222, "适合");
		_menu3A.CheckMenuItem(222, properties.wallpaperdisplay==1);
		_menu3A.AppendMenuItem(MF_STRING, 223, "拉伸");
        ...
        switch(true) {
			...
			case (idx == 3031):
				try {
					customControlDetails("自定义显示字段"
										,"<div class='titleBig'>自定义显示字段</div><div class='separator'></div><br/>为显示的每一行输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.<br/><a href=\"http://tinyurl.com/lwhay6f\" target=\"_blank\">点击这里</a> 了解关于工具栏标题格式信息. (http://tinyurl.com/lwhay6f)<br/>"
										,''
										,'第一行 (默认 %title% - %artist%):##第二行, 选项启用时显示 (默认 %album% (%date%)):'
...
function SimpleButton(x, y, w, h, text, tooltip_text, fonDown, fonUp, fonDbleClick, N_img, H_img, state) {
    ...
    this.draw = function (gr) {
        ...
        else if(this.text=="播放" && (layout_state.isEqual(0) || showtrackinfo_small.isActive())){
        ...
        } else if(this.text=="音量" && !properties.volumeA1waysVisible){
...
function get_repeat_tooltip(){
	if(plman.PlaybackOrder==0 && properties.playandrandom) {
		var tooltip_text = '播放列表结束后播放相同流派音轨';
	} else if(plman.PlaybackOrder==0){
		var tooltip_text = '默认顺序';
	} else if(plman.PlaybackOrder==1) {
		var tooltip_text = '重复全部';
	} else if(plman.PlaybackOrder==2) {
		var tooltip_text = '重复当前';
