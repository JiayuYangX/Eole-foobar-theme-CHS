...
oPlaylistManager = function(name) {
    ...
    this.draw = function(gr) {
        if(this.offset > 0) {
            ...
            if(cPlaylistManager.blink_counter > -1) {
                if(cPlaylistManager.blink_row == 0) {
					gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                    if(cPlaylistManager.blink_counter <= 6 && Math.floor(cPlaylistManager.blink_counter / 2) == Math.ceil(cPlaylistManager.blink_counter / 2)) {
                    ...
                    gr.GdiDrawText("发送到 ..." , g_font.italicplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                };
            } else {
                if(this.activeRow == 0) {
                    gr.GdiDrawText("+ 发送到新播放列表" , g_font.boldplus1, txt_color, cx + bg_margin_left + txt_margin, this.y + cPlaylistManager.topbarHeight - 30, cw - bg_margin_left*2 - txt_margin*2 - tw - this.scr_w, ch, DT_LEFT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
...
oFilterBox = function() {
    ...
    this.getImages = function() {
		...
		this.search_bt = new button(this.images.search_icon, this.images.search_icon, this.images.search_icon,"search_bt", "过滤项目");
    ...
    this.reset_layout = function() {
		...
		var item_txt = new Array("", "专辑...", "艺术家...", "流派...");
...
oTagSwitcherBar = function() {
    this.setItems_infos = function(){
		...
		this.items_txt = new Array("树形媒体库","专辑", "艺术家", "流派");
		this.items_tooltips = new Array("树形媒体库","专辑过滤", "艺术家过滤", "流派过滤");
    ...
    this.setHideButton = function(){
		...
		this.hide_bt = new button(this.hide_bt_off, this.hide_bt_ov, this.hide_bt_ov,"hide_filters", "隐藏左侧菜单");
	}
    this.getImages = function() {
		if(properties.darklayout) icon_theme_subfolder = "\\white";
		else icon_theme_subfolder = "";
		...
		this.search_history_bt = new button(this.images.search_history_icon, this.images.search_history_hover_icon, this.images.search_history_hover_icon,"search_history_bt","更改分组");		
	};
	this.getImages();
	this.on_init = function() {
		this.setItems_infos();
	...
	this.drawSwitchMenu = function(x, y) {
		...
		basemenu.AppendMenuItem(MF_GRAYED, 0, "分组到:");
		...
		//basemenu.AppendMenuSeparator();
		//basemenu.AppendMenuItem(MF_STRING, 0, "自定义分组");
...
oTagSwitcherBar_old = function() {
    this.setItems_infos = function(){
		...
		this.items_width = new Array(0, 0, 0, 0);
		this.items_x = new Array(0, 0, 0, 0);
		this.items_txt = new Array("T","专辑", "艺术家", "流派");
		this.items_tooltips = new Array("树形媒体库","专辑过滤", "艺术家过滤", "流派过滤");
		properties.album_label = this.items_txt[1];
		properties.artist_label = this.items_txt[2];
		properties.genre_label = this.items_txt[3];		
...
oBrowser = function(name) {
    ...
    this.populate = function(is_first_populate, call_id, force_sourceMode, force_playlist) {
        ...
        console.log("--> 过滤器填充顺序:"+properties.filterOrder+" 父面板:"+properties.ParentName+" call_id:"+call_id);
        ...
        if(repaint_main || !repaintforced){
			if(this.groups.length>0){
							...
							case 3:
								...
								if(properties.displayMode == 1) { // panelMode = 1 (Art + bottom labels)
									...
									if(properties.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
										try{
											gr.GdiDrawText("全部项目", g_font.normal, txt_color1, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth), coverWidth, this.firstRowHeight, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
											gr.GdiDrawText(""+(total-1)+" 项目", g_font.italicmin1, txt_color2, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth + this.firstRowHeight), coverWidth, this.secondRowHeight, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
										} catch(e) {console.log(e)}
									} else {
										if(!this.customGroups){
											if(this.groups[i].album == "?") {
												if(this.groups[i].count > 1) {
													var album_name = (this.groups[i].tracktype != 3 ? "(单曲)" : "(网络收音机)");
												} else {
											...
											this.groups[i].secondRow = this.groups[i].count+" 项目"+(this.groups[i].count>1?'':'');
										}
										...
										try{
											if(properties.tagMode == 1) {
											  ...
											  gr.GdiDrawText(''+this.groups[i].count+' 音轨', font2, txt_color2, ax + Math.round((aw - coverWidth) / 2), (coverTop + 5 + coverWidth +  + this.firstRowHeight), coverWidth, this.secondRowHeight, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
											};
										} catch(e) {}
									};
									...
									// draw text
									if(properties.showAllItem && i == 0 && total > 1) { // aggregate item ( [ALL] )
										this.groups[i].firstRow = "全部 "+(this.customGroups?"items":'albums');
										this.groups[i].secondRow = this.groups[i].count+" 项目"+(this.groups[i].count>1?'':'');
									} else {
							...
							case 2:
								...
								if(properties.showAllItem && i==0 && total > 1) { // [ALL] item
									switch(properties.tagMode) {
										case 1: // album
											try{
												gr.GdiDrawText("全部项目", g_font.normal, txt_color1, this.margin_left + ax + coverWidth + this.marginCover*2, ay - properties.textLineHeight, aw - coverWidth - this.marginCover*3-this.textMarginRight - items_counter_width, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
												...
												gr.GdiDrawText("("+(total-1)+(!(properties.tf_groupkey_album == properties.tf_groupkey_album_default && properties.album_customGroup_label == "")?' 分组)':" 专辑)"), g_font.min1, txt_color2, this.margin_left + ax + coverWidth + this.marginCover*2, ay + properties.textLineHeight, aw - coverWidth - this.marginCover*3-this.textMarginRight, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
											} catch(e) {}
											break;
										case 2: // artist
											try{
												gr.GdiDrawText("全部 ("+(total-1)+(!(properties.tf_groupkey_artist == properties.tf_groupkey_artist_default && properties.artist_customGroup_label == "")?' 分组)':" 艺术家)"), g_font.normal, txt_color1, this.margin_left + ax + coverWidth + this.marginCover*2, ay, aw - coverWidth - this.marginCover*3-this.textMarginRight - items_counter_width, ah, DT_LEFT | DT_VCENTER | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
				...
				if(properties.displayMode > 0) {
					...
					var line_width = Math.min(130,Math.round(this.w-40));
					var py = this.y + Math.round(this.h  / 2)-1;
					gr.GdiDrawText("正在载入...", g_font.plus4, colors.normal_txt, this.x, py - 40, this.w, 36, DT_CENTER | DT_BOTTOM | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
					gr.FillSolidRect(this.x+Math.round(this.w/2-line_width/2),py, line_width, 1, colors.border);
					gr.GdiDrawText("过滤", g_font.italicplus1, colors.faded_txt, this.x, py + 6, this.w, 20, DT_CENTER | DT_TOP | DT_CALCRECT | DT_END_ELLIPSIS | DT_NOPREFIX);
			};
						...
						if((Math.abs(x - this.drag_clicked_x) > 10 || Math.abs(y - this.drag_clicked_y) > 10) && this.h > cPlaylistManager.rowHeight * 6) {
							...
							if (typeof this.groups[this.selectedIndex].cover_img !== "undefined" && this.groups[this.selectedIndex].cover_img!==null) {
								...
								var options = {
									...
									use_album_art : false,
									use_theming : false,
									custom_image : createDragText(this.groups[this.selectedIndex].tooltipText, this.groups[this.selectedIndex].count+" 音轨", 220),
								}
							}
	...
	this.item_context_menu = function(x, y, albumIndex) {
		...
		if(fb.IsPlaying && this.current_sourceMode==0){
			_menu.AppendMenuItem(MF_STRING, 1011, "定位到播放分组");
			_menu.AppendMenuSeparator();
		}
		if(properties.displayMode==1 || properties.displayMode==2 || properties.displayMode==3){
			_menu.AppendMenuItem(MF_STRING, 1010, "刷新此图像");
		}

		_child02.AppendTo(_menu, MF_STRING, "发送到...");
		...
		if(utils.IsKeyPressed(VK_SHIFT)) {
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 1001, "属性 ");
			_menu.AppendMenuItem(MF_STRING, 1002, "配置...");
			_menu.AppendMenuSeparator();
			_menu.AppendMenuItem(MF_STRING, 1003, "重载入");
		}
    ...
    this.settings_context_menu = function(x, y) {
            ...
            _menu13.AppendMenuItem(MF_STRING, 113, "默认 (%genre%)");
            ...
            _menu13.AppendMenuItem(MF_STRING, 116, "自定义标题格式...");
            ...
            _menu12.AppendMenuItem(MF_STRING, 112, "默认 (%artist%)");
            ...
            _menu12.AppendMenuItem(MF_STRING, 115, "自定义标题格式...");
            ...
            _menu11.AppendMenuItem(MF_STRING, 111, "默认 (%album%)");
            ...
            _menu11.AppendMenuItem(MF_STRING, 114, "自定义标题格式...");
			...
			if(properties.showLibraryTreeSwitch) {
				_menu.AppendMenuItem(MF_STRING, 990, "切换到树形媒体库");
				_menu.AppendMenuSeparator();
			}
            _menu13.AppendTo(_menu1,(properties.tagMode==3)?MF_CHECKED:MF_STRING, "预设 1 ("+properties.genre_label.toUpperCase()+")");
            _menu12.AppendTo(_menu1,(properties.tagMode==2)?MF_CHECKED:MF_STRING, "预设 2 ("+properties.artist_label.toUpperCase()+")");
            _menu11.AppendTo(_menu1,(properties.tagMode==1)?MF_CHECKED:MF_STRING, "预设 3 ("+properties.album_label.toUpperCase()+")");

            _menu1.AppendTo(_menu,MF_STRING, "分组/排序按");

            _menu2.AppendMenuItem(MF_STRING, 913, "标签切换栏");
            _menu2.CheckMenuItem(913, properties.showTagSwitcherBar);
			if(main_panel_state.isEqual(0)){
				_menu2.AppendMenuItem(MF_STRING, 914, "隐藏菜单按钮");
				_menu2.CheckMenuItem(914, properties.showFiltersTogglerBtn);
			}
            _menu2.AppendMenuItem(MF_STRING, 910, "搜索栏");
            _menu2.CheckMenuItem(910, properties.showHeaderBar);
            _menu2.AppendMenuSeparator();

            _menu2.AppendMenuItem(MF_STRING, 900, "仅文本");
            _menu2.AppendMenuItem(MF_STRING, 902, "文本 (右侧专辑封面)");
            _menu2.AppendMenuItem(MF_STRING, 901, "专辑封面 (底部文本)");
            _menu2.AppendMenuItem(MF_STRING, 903, "方形专辑封面");
			_menu2.AppendMenuItem(MF_STRING, 1806, "方形专辑封面 (无文本)");
            ...
            _menu2.AppendMenuSeparator();
			if(properties.tagMode!=1){
				_menu2.AppendMenuItem(MF_STRING, 918, "专辑封面退后");
				_menu2.CheckMenuItem(918, properties.AlbumArtFallback);
			}
			_menu2.AppendMenuItem(MF_STRING, 904, "圆形封面");
            _menu2.CheckMenuItem(904, properties.circleMode);
            _menu2.AppendMenuItem(MF_STRING, 911, "项目合计");
            _menu2.CheckMenuItem(911, properties.showAllItem);
            _menu2.AppendMenuItem(MF_STRING, 912, "项目计数 (显示文本)");
            _menu2.CheckMenuItem(912, properties.drawItemsCounter);
            _menu2.AppendMenuItem(MF_STRING, 916, "显示工具条提示");
			...
			_rowHeight.AppendMenuItem(MF_STRING, 1000, "减少");
			_rowHeight.AppendMenuSeparator();
			_rowHeight.AppendMenuItem(MF_DISABLED, 0, "提示: 按住 SHIFT 并使用鼠标滚轮");
			_rowHeight.AppendMenuItem(MF_DISABLED, 0, "在皮肤面板的任何位置滚动滚轮 !");
			if(properties.displayMode==0 || properties.displayMode==2){
				_rowHeight.AppendTo(_menu,MF_STRING, "行高");
			} else {
				_rowHeight.AppendTo(_menu,MF_STRING, "列宽");
			}

			if(properties.ParentName=='Library'){
				var _panelWidth = window.CreatePopupMenu();
				_panelWidth.AppendMenuItem(MF_STRING, 1030, "增加宽度");
				_panelWidth.AppendMenuItem(MF_STRING, 1031, "减少宽度");
				_panelWidth.AppendMenuItem(MF_STRING, 1033, "自定义宽度...");
				_panelWidth.AppendMenuItem(MF_STRING, 1032, "重置");
				_panelWidth.AppendTo(_menu,MF_STRING, "面板宽度");
			}
			...
			//FiltersMenu.AppendTo(_menu, MF_STRING, "过滤");

            /*_menu.AppendMenuSeparator();
            _menu.AppendMenuItem(MF_STRING, 914, "Enable Disk Image Cache");
            _menu.CheckMenuItem(914, globalProperties.enableDiskCache);
            _menu.AppendMenuItem((globalProperties.enableDiskCache)?MF_STRING:MF_GRAYED, 915, "启动时加载所有封面");
			...
			_menu.AppendMenuItem(MF_STRING, 2997, "拖放时显示播放列表面板");
			_menu.CheckMenuItem(2997, properties.DropInplaylist);
			
			_menu.AppendMenuItem(MF_STRING, 2998, "排序忽略/les/los");
			_menu.CheckMenuItem(2998, properties.removePrefix);			

			if(main_panel_state.isEqual(1)) {
				_menu.AppendMenuSeparator();
				_menu.AppendMenuItem((filters_panel_state.isActive() ? MF_STRING : MF_GRAYED | MF_DISABLED), 2993, "启用第 1 个过滤");
				_menu.CheckMenuItem(2993, (filter1_state.isActive()));
				_menu.AppendMenuItem((filters_panel_state.isActive() ? MF_STRING : MF_GRAYED | MF_DISABLED), 2994, "启用第 2 个过滤");
				_menu.CheckMenuItem(2994, (filter2_state.isActive()));
				_menu.AppendMenuItem((filters_panel_state.isActive() ? MF_STRING : MF_GRAYED | MF_DISABLED), 2995, "启用第 3 个过滤");
				_menu.CheckMenuItem(2995, (filter3_state.isActive()));
				_menu.AppendMenuSeparator();
				if(!filters_panel_state.isMaximumValue())
					_menu.AppendMenuItem(MF_STRING, 2992, "隐藏过滤");
				else
					_menu.AppendMenuItem(MF_STRING, 2996, "显示底部播放列表");
			}
			...
			if(utils.IsKeyPressed(VK_SHIFT)) {
				_menu.AppendMenuSeparator();
				_menu.AppendMenuItem(MF_STRING, 991, "属性 ");
				_menu.AppendMenuItem(MF_STRING, 992, "配置...");
				_menu.AppendMenuSeparator();
				_menu.AppendMenuItem(MF_STRING, 993, "重载入");
			}
                    ...
                    switch(properties.tagMode) {
                        case 1:
							try {
								...
								/*new_TFgrouping = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.", "Album grouping", properties.tf_groupkey_album, true);
                        ...
                        case 2:
							try {
								...
								/*new_TFgrouping = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.", "Artist grouping", properties.tf_groupkey_artist, true);
                        ...
                        case 3:
							try {
								...
								/*new_TFgrouping = utils.InputBox(window.ID, "输入一个标题格式脚本.\n您可以在这里使用完整的 foobar2000 标题格式语法.\n\n参考 http://tinyurl.com/lwhay6f\n了解关于工具栏标题格式信息.", "Genre grouping", properties.tf_groupkey_genre, true);
								if (!(new_TFgrouping == "" || typeof new_TFgrouping == 'undefined')) {
...
function setButtons(){
	buttons = {
		filtersToggle: new SimpleButton(18, 7, 35, 40, "filtersToggle", "减小过滤", function () {
