...
var globalProperties = {
	...
	selection_playlist : "选择媒体库",
	playing_playlist : "播放媒体库",
	filter_playlist : "过滤结果",	
	whole_library : "全部媒体库",
	media_library : "媒体库",
...
function chooseMemorySettings(title, top_msg, bottom_msg, dialog_name, inter_text){
	...
	utils.ShowHtmlDialog(window.ID, htmlCode(skin_global_path+"\\html",dialog_name+".html"), {
		data: [title, top_msg, '取消', ok_callback,'0 - 最小值##1 - 封面加载保留到内存中##2 - 启动时加载所有封面##3 - 启动时加载所有封面和艺术家缩略图',globalProperties.mem_solicitation,bottom_msg,globalProperties.coverCacheWidthMax,inter_text],
	});
}
...
function customFilterGrouping(title, top_msg, bottom_msg, input_default_values, input_labels){
	...
	utils.ShowHtmlDialog(window.ID, htmlCode(skin_global_path+"\\html","InputDialog.html"), {
		data: [title, top_msg, '取消', ok_callback,bottom_msg,input_default_values,input_labels],
	});
}
    ...
    this.CreateTextFile = function (path) {
		try {
           return this.fileObject.CreateTextFile(path);
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"CreateTextFile call, "+path, "错误");
            console.log(e)
        }
    };
    ...
    this.MoveFile = function (target,dest) {
		try {
           return this.fileObject.MoveFile(target,dest);
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"MoveFile call, from "+target+" to "+dest, "错误");
			console.log(e);
		}
    };
    ...
    this.DeleteFolder = function (path,force) {
        try {
           return this.fileObject.DeleteFolder(path,force);
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"DeleteFolder call, "+path+" force:"+force, "错误");
			console.log(e);
		}
    };
...
oUIHacks = function () {
	...
	this.SetPseudoCaption = function (x, y, w, h) {
        try {
           return this.activeXObject.SetPseudoCaption(x, y, w, h);
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks SetPseudoCaption x:"+x+" y:"+y+" w:"+w+" h:"+h, "错误");
			console.log(e);
		}
	}
	...
	this.setFullscreenState = function (new_state) {
        try {
           this.activeXObject.FullScreen = new_state;
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setFullscreenState "+new_state, "错误");
			console.log(e);
		}
	}
	...
	this.setMainWindowState = function (new_state) {
        try {
           this.activeXObject.MainWindowState = new_state;
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setMainWindowState "+new_state, "错误");
			console.log(e);
		}
	}
	...
	this.setMinHeight = function (height) {
        try {
           this.activeXObject.MinSize.Height = height;
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setMinHeight "+height, "错误");
			console.log(e);
		}
	}
	...
	this.setMaxHeight = function (height) {
        try {
           this.activeXObject.MaxSize.Height = height;
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setMaxHeight "+height, "错误");
			console.log(e);
		}
	}
	this.setAero = function (top,right,bottom,left) {
        try {
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setAero left:"+left+" top:"+top+" right:"+right+" bottom:"+bottom, "错误");
			console.log(e);
		}
	}
	...
	this.setFrameStyle = function (style) {
        try {
           this.activeXObject.FrameStyle = style;
			...
			fb.ShowPopupMessage('哦, 好像出错了\n\n'+"UIHacks setFrameStyle "+style, "错误");
			console.log(e);
		}
	}
}
...
function sendandplayPlaybackPlaylist(items, play_metadb){
	...
	if(pidx_playing>-1) {
		...
		if(play_metadb)
			fb.RunContextCommandWithMetadb("添加到播放队列", play_metadb);
		else
...
function enableDiskCacheGlobally(){
	...
	if(globalProperties.enableDiskCache) HtmlMsg("关于磁盘镜像缓存的说明", "磁盘图像缓存是一点一点建立起来的: 当显示封面时, 如果缓存中尚未存储封面, 则会将其添加到缓存中.\n\n磁盘镜像缓存基于 %album artist% & %album% 标签.\n\n更新现有封面后, 必须在 foobar 中手动刷新, 在需要刷新的封面上单击右键, 就会出现一个菜单项.", "确定")
	else  HtmlMsg("关于磁盘镜像缓存的说明", "警告: 在未启用磁盘映像缓存的情况下, foobar 的运行速度可能会变慢.\n\n重启 foobar 以完全禁用.", "确定");
}
function enableCoversAtStartupGlobally(){
	...
	if(globalProperties.load_covers_at_startup) HtmlMsg("关于磁盘镜像缓存的说明", ((!globalProperties.enableDiskCache)?"如果磁盘镜像缓存被启用并且已经建立, 这个选项会更好地发挥作用 (检查下面的选项).\n\n":"")+"启用该选项时 Foobar 的内存占用率较高, 因为所有的封面都被加载到内存中, 但如果您的资料库不是很大, 应该没有问题.\n\n如果更新现有封面, 需要刷新缓存. 在封面上点击右键, 会弹出一个相关的菜单项..\n\n磁盘图像缓存基于 %album artist% & %album% 标签.\n\n重新启动 foobar 开始加载所有封面.", "确定")
}
function enableArtistImgAtStartupGlobally(){
	...
	if(globalProperties.load_artist_img_at_startup) HtmlMsg("关于磁盘镜像缓存的说明",((!globalProperties.enableDiskCache)?"如果磁盘镜像缓存被启用并且已经建立, 这个选项会更好地发挥作用 (检查下面的选项).\n\n":"")+"启用该选项后, Foobar 的内存使用率会更高, 因为所有艺术家的缩略图都会加载到内存中.\n\n如果要更新艺人缩略图, 必须在 foobar 中手动刷新, 在需要刷新的艺人缩略图上单击右键, 就会出现相应的菜单项.\n\n磁盘图像缓存基于 %album artist% 标签.\n\n重启 foobar 开始加载所有封面.", "确定")
}
...
function createGenrePopupMenu(firstFile, checked_item, genrePopupMenu){
	...
	if(firstFile){
		...
		if (DefaultGenreIndex>0 && !showBelow) {
			genrePopupMenu.AppendMenuItem(MF_STRING, DefaultGenreIndex+999, g_genre_cache.genreList[DefaultGenreIndex-1][0].replace("&","&&")+"   (从首个文件路径估算)");
	...
	if(g_genre_cache.genreList.length==0){
		if(g_genre_cache.initialized === false) genrePopupMenu.AppendMenuItem(MF_DISABLED, 0, "目前正在建立流派列表. 应该在几秒内准备完毕.");
		else {
			genrePopupMenu.AppendMenuItem(MF_DISABLED, 0, "库中弹出的列表目前为空.");
			genrePopupMenu.AppendMenuItem(MF_DISABLED, 0, "注意: 列表将在下次启动时刷新.");
			genrePopupMenu.AppendMenuSeparator();
			genrePopupMenu.AppendMenuItem(MF_STRING, 10000, "立即刷新");
		}
	} else {
		for (var i = 0; i < g_genre_cache.genreList.length; i++) {
	...
	if(firstFile){
		if(firstFileGenre!="") var currentGenre="当前流派: '"+firstFileGenre.replace("&","&&")+"'";

		genrePopupMenu.AppendMenuSeparator();
		genrePopupMenu.AppendMenuItem(MF_GRAYED, 0, currentGenre);
		if(showBelow && firstFileGenre!="") genrePopupMenu.AppendMenuItem(MF_GRAYED, 0, "与路径猜测的流派相同");
	}
	...
	if(!globalProperties.use_ratings_file_tags && foo_playcount) {
		...
		if (new_rating != old_rating) {
			if (metadb) {
				...
				var bool = fb.RunContextCommandWithMetadb("评级/" + ((new_rating == 0) ? "<未设置>" : new_rating), metadb,8);
				if(bool) return new_rating;
				else return false;
			};
		} else {
			window.NotifyOthers('rating_updated',metadb);
			var bool = fb.RunContextCommandWithMetadb("评级/<未设置>", metadb,8);
			if(bool) return 0;
			else return false;
		};
	} else {
...
function SetGenre(GenreNumber, plist_items, max_items, clean_file){
    ...
    if(plist_items.Count>max_items) {
         var result = HtmlMsg("错误", "当前播放列表包含超过 "+max_items+" 文件. 请使用标准属性对话框.", "确定");
		 return false;
    } else {
		...
		HtmlDialog("请确认", QuestionString, '是', '否', update_confirmation);
    }
	return false;
}
...
function removeItems(plist_items, plist_Idx, ask_for_confirmation){
	...
	if(!plist_items) {
        ...
        var QuestionString = "移除所选 ("+plman.PlaylistItemCount(plist_Idx)+") 全部文件 当前播放列表 > ("+plman.GetPlaylistName(plist_Idx)+") ?";
		...
		if(ask_for_confirmation){
			...
			var QuestionString = "移除所选 "+plist_items.Count+" 文件"+((plist_items.Count>1)?" ":"")+" 当前播放列表 > ("+plman.GetPlaylistName(plist_Idx)+") ?";
			HtmlDialog("请确认", QuestionString, '是', '否', delete_confirmation);
    ...
    if(g_files.FileExists(filename)) {
		try {
			...
			if(delete_at_startup && delete_at_startup==true)
				HtmlMsg("无法删除此文件", "缓存封面无法删除.\n尝试关闭 foobar 并手动删除以下文件 :\n\n"+cover_img_cache + "\\" + crc, "确定");
			else
				HtmlMsg("无法删除此文件", "目前无法从 foobar 中刷新缓存的封面图像 (文件正在使用中), 但 foobar 将尝试在下次启动时刷新", "确定");
		};
        return true;
    };
    return false;
...
function delete_full_cache(){
	if(globalProperties.deleteDiskCache) {
		...
		var QuestionString = "是否完全重置图像缓存 ?\n\n如果您确认, 图像缓存将在下次启动时被刷新. Foobar 将退出, 请手动重新启动.";
		HtmlDialog("请确认", QuestionString, '是', '否', delete_confirmation);
	}
}
...
function createDragImg(img, cover_size, count){
	...
	gb.GdiDrawText(count+" 音轨", g_font.italicmin1,  colors.dragimage_text, left_padding, top_padding+cover_size-text_height, cover_size, text_height, DT_CENTER | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
	drag_img.ReleaseGraphics(gb);
...
function openCoverFullscreen(metadb){
	...
	try {
		...
		//HtmlMsg("错误", "Image not found, this cover is probably embedded inside the audio file."+filepath,"确定");
	}		
}
// Debugger functions
...
function JSON_stringify(info) {
	try {
	   return JSON.stringify(info);
		...
		fb.ShowPopupMessage('哦, 好像出错了\n\n'+"JSON_stringify "+info, "错误");
		console.log(e)
	}
}
function CreatePlaylist(pl_place, pl_name) {
	try {
		result = utils.InputBox(window.ID, "命名播放列表名称.", "创建播放列表", "播放列表", true);
		if (result){
			if (result == "") {
				plman.CreatePlaylist(pl_place, "播放列表");
