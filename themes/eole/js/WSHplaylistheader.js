...
buttons = {
    Settings: new SimpleButton(0, 8, 21, 21, "Settings", "设置...", function () {
    ...
    filtersToggle: new SimpleButton(18, 8, 50, 21, "filtersToggle", "增大过滤", function () {
...
function draw_settings_menu(x, y) {
    ...
    if(!nowplayingplaylist_state.isActive()) basemenu.AppendMenuItem(MF_STRING, 4999, "显示右侧播放列表");
	else basemenu.AppendMenuItem(MF_STRING, 4999, "隐藏右侧播放列表");
    if(!filters_panel_state.isMaximumValue()) basemenu.AppendMenuItem(MF_STRING, 4996, "隐藏底部播放列表");
	else {basemenu.AppendMenuItem(MF_STRING, 4996, "显示底部播放列表");}
	...
	basemenu.AppendMenuItem(MF_STRING, 4989, "显示切换按钮");
...
function draw_menu(x, y) {
    ...
    if(!nowplayingplaylist_state.isActive()) panelsMenu.AppendMenuItem(MF_STRING, 4999, "显示右侧播放列表");
	else panelsMenu.AppendMenuItem(MF_STRING, 4999, "隐藏右侧播放列表");
    if(!filters_panel_state.isMaximumValue()) panelsMenu.AppendMenuItem(MF_STRING, 4996, "隐藏底部播放列表");
	else panelsMenu.AppendMenuItem(MF_STRING, 4996, "显示底部播放列表");
	...
	FiltersMenu.AppendMenuItem((filters_panel_state.isActive() ? MF_STRING : MF_GRAYED | MF_DISABLED), 4995, "启用第 3 个过滤");
	FiltersMenu.CheckMenuItem(4995, (filter3_state.isActive()));
	FiltersMenu.AppendTo(panelsMenu, MF_STRING, "过滤");

	panelsMenu.AppendMenuSeparator();
	panelsMenu.AppendMenuItem(MF_STRING, 4989, "显示切换按钮");
	panelsMenu.CheckMenuItem(4989, properties.displayToggleBtns);
	panelsMenu.AppendTo(basemenu, MF_STRING, "面板位置");
	...
	if(!plman.IsAutoPlaylist(plman.ActivePlaylist)){
		...
		SortMenu.AppendMenuItem(MF_STRING, 3002, "专辑/音轨号");
		SortMenu.AppendMenuItem(MF_STRING, 3003, "音轨号");
		SortMenu.AppendMenuItem(MF_STRING, 3004, "文件路径");
		SortMenu.AppendMenuItem(MF_STRING, 3005, "标题");
		SortMenu.AppendMenuItem(MF_STRING, 3006, "日期");
		SortMenu.AppendMenuItem(MF_STRING, 3007, "最短到最长");
		SortMenu.AppendMenuItem(MF_STRING, 3008, "最长到最短");
		SortMenu.AppendMenuItem(MF_STRING, 3009, "评级");
		SortMenu.AppendMenuItem(MF_STRING, 3010, "自定义标题格式...");
		SortMenu.AppendMenuSeparator();
		SortMenu.AppendMenuItem(MF_STRING, 3011, "随机");
	}
	
    basemenu.AppendMenuSeparator();
    basemenu.AppendMenuItem(MF_STRING, 3000, "全部选择");
    basemenu.AppendMenuItem(MF_STRING, 2998, "全部移除");
	...
	if(metadblist_selection.Count > 0) {
		...
		Context_root.AppendTo(basemenu, MF_STRING, "选择 ("+metadblist_selection.Count+" 项目"+((metadblist_selection.Count>1)?" ":"")+")");
		Context.InitContext(metadblist_selection);
		Context.BuildMenu(Context_root, 2, -1);
		Context_root.AppendMenuItem(MF_STRING, 2997, "移除");
	}
	basemenu.AppendMenuItem(MF_STRING, 2000, "音轨属性");


	if(utils.IsKeyPressed(VK_SHIFT)) {
		basemenu.AppendMenuSeparator();
		basemenu.AppendMenuItem(MF_STRING, 5100, "属性 ");
		basemenu.AppendMenuItem(MF_STRING, 5101, "配置...");
		basemenu.AppendMenuSeparator();
		basemenu.AppendMenuItem(MF_STRING, 5102, "重载入");
	}
	...
	case (idx == 1999):
		...
		break;
	case (idx == 2000):
		fb.RunContextCommandWithMetadb("属性", plman.GetPlaylistItems(plman.ActivePlaylist), 0);
		break;
    case (idx == 2997):
...
playlistInfo = function(){
	...
	this.setTexts = function() {
		...
		if(displayed_count>0) {
			...
			txt_month = (totalMth > 0) ? `${totalMth} 月${totalMth > 1 ? '' : ''}, `: '';
			...
			this.time_txt = `${txt_month}${txt_week}${txt_day}${txt_hour}${txt_mins}${totalS}秒`;
			this.items_txt=displayed_count+' 项目 ';
			...
			if(filtered_playlist!="" && filtered_playlist!=globalProperties.selection_playlist){				
				this.main_txt='播放列表 : '+plman.GetPlaylistName(properties.filtred_playlist_idx)+' (Filtered)';	
				...
				this.main_txt='播放列表 : '+this.playlist_name;
			} else if(this.albums!=""){
				if(this.artists!="") this.main_txt=this.albums+' - '+this.artists
				else this.main_txt='各类艺术家 - '+this.albums;
			} else if(this.artists!="") {
				this.main_txt=this.artists+' - 若干专辑';
			} else if(this.genres!="") {
			...
			this.main_txt='播放列表 : '+this.playlist_name;
			this.items_txt='空播放列表';
		}
		this.items_width = -1;
		this.main_txt_width = -1;
	}
	this.refresh = function(populate_list,call_id) {
		console.log("--> 刷新主播放列表标头 populate_list:"+populate_list+" call_id:"+call_id)
		if(populate_list) {
			this.populate();
		} else {
			if(this.selected_count>1){
...
function on_mouse_mbtn_down(x, y, mask) {
    fb.RunContextCommandWithMetadb("属性",plman.GetPlaylistItems(plman.ActivePlaylist));
}
