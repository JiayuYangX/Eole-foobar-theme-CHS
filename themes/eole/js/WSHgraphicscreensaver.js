...
var properties = {
    ...
    TFgrouping_default_filterbox: "%album artist% ^^ %album%[' - 光碟 '%discnumber%] ^^ %genre% ^^ %date% ^^ %title%",
    TFgrouping_default: "%title% ^^ %album artist% ^^ %album%[' - 光碟 '%discnumber%]",
    ...
    TFtitle: "%artist% ^^ [%discnumber%.] ^^ %tracknumber% ^^ %title% ^^ $if2(" + (globalProperties.use_ratings_file_tags ? "$meta(rating)" : "%rating%") + ",0) ^^ $if(%length%,%length_seconds%,'广播') ^^ $if2(%play_counter%,$if2(%play_count%,0))",
    TFshowlist: "%album artist% ^^ %album% ^^ [' - 光碟 '%discnumber%] ^^ %date% ^^ %genre%",
...
oShowList = function(parentPanelName) {
    ...
    this.getHeaderInfos = function(EvalWithMetadb) {
		...
		if(EvalWithMetadb){
			...
			this.date = Tags[3];
			this.genre = Tags[4];
			this.total_tracks = this.pl.Count+(this.pl.Count>1?" 音轨":" 音轨");
		} else {
...
oHeaderbar = function(name) {
	...
	this.setDisplayedInfo = function() {
		...
		if(brw.finishLoading) {
			if(brw.playlistItemCount) {
				...
				this.itemsTxt=',  '+brw.playlistItemCount+' 音轨'+((brw.playlistItemCount>1)?" ":"")+',  '+brw.groups_draw.length+' 分组 '+((brw.groups_draw.length>1)?" ":"");
	...
	this.draw_header_menu = function(x,y){
		...
		if(!plman.IsAutoPlaylist(plman.ActivePlaylist)){
			...
			SortMenu.AppendMenuItem(MF_STRING, 3000, "不排序 (播放顺序)");
			SortMenu.AppendMenuItem(MF_STRING, 3001, "艺术家/专辑/音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 3002, "标题");
			SortMenu.AppendMenuItem(MF_STRING, 3003, "音轨号");
			SortMenu.AppendMenuItem(MF_STRING, 3004, "日期");
			SortMenu.AppendMenuItem(MF_STRING, 3005, "加入媒体库日期 (新加入优先)");
		...
		basemenu.AppendMenuItem(MF_STRING, 35, "音轨属性");
...
function draw_settings_menu(x,y){
	...
	_menu.AppendMenuItem(MF_STRING, 10, "始终跟随播放");
	_menu.CheckMenuItem(10, properties.followNowPlaying);
	_menu.AppendMenuSeparator();
	_menu.AppendMenuItem(MF_STRING, 14, "启用磁盘封面缓存");
	_menu.CheckMenuItem(14, globalProperties.enableDiskCache);
	_menu.AppendMenuItem((globalProperties.enableDiskCache)?MF_STRING:MF_GRAYED, 12, "启动时加载所有封面");
	_menu.CheckMenuItem(12, globalProperties.load_covers_at_startup);
	_menu.AppendMenuItem(MF_STRING, 26, "显示封面加载进度");
	_menu.CheckMenuItem(26, properties.show_covers_progress);
	_menu.AppendMenuSeparator();
	_menu.AppendMenuItem(MF_STRING, 27, "显示标题栏");
	_menu.CheckMenuItem(27, properties.showheaderbar);
	_menu.AppendMenuItem((!properties.showheaderbar)?MF_DISABLED:MF_STRING, 30, "显示过滤栏");
	_menu.CheckMenuItem(30, properties.showFilterBox);
	_menu.AppendMenuItem(MF_STRING, 25, "在专辑封面上显示日期");
	_menu.CheckMenuItem(25, properties.showdateOverCover);
	_menu.AppendMenuItem(MF_STRING, 37, "圆形封面");
	...
	_menuTracklist.AppendMenuItem(MF_STRING, 28, "音轨显示艺术家");
	...
	_menuTracklist.AppendMenuItem(MF_STRING, 29, "右侧显示封面");
	...
	_menuBackground.AppendMenuItem(MF_STRING, 15, "跟随专辑封面背景 (主颜色)");
	_menuBackground.AppendMenuItem(MF_STRING, 16, "跟随专辑封面背景 (模糊)");
	_menuBackground.AppendMenuItem(MF_STRING, 17, "跟随专辑封面背景 (混合)");
	...
	_menu.AppendMenuSeparator();
	_menu.AppendMenuItem(MF_STRING, 224, "浅色主题");
	_menu.CheckMenuItem(224, !properties.darklayout);
	_menu.AppendMenuItem(MF_STRING, 225, "深色主题");
...
oBrowser = function(name) {
   ...
   timers.firstPopulate = setTimeout(function(){
	...
	this.FormatTime = function(time) {
		...
		if (time > 0) {
			...
			txt_week = (totalW > 0) ? `${totalW} 星期${totalW > 1 ? '' : ''}, ` : '';
			txt_day = (totalD > 0) ? `${totalD} 天${totalD > 1 ? '' : ''}, ` : '';
			txt_hour = (totalH > 0) ? `${totalH} 小时${totalH > 1 ? '' : ''}, ` : '';
			txt_mins = (totalM > 0) ? `${totalM} 分${totalM > 1 ? '' : ''}, ` : '';
   ...
   this.get_metrics = function(gr){
    ...
    this.get_albums = function(start, str_comp){
        ...
        while(k < total){
            ...
            if(string_compare != temp){
				...
				if(i>0) {
					...
					if(this.custom_secondRow) this.groups[i-1].secondRow = this.groups[i-1].pl.Count+(this.groups[this.groups.length-1].pl.Count>1?" 音轨":" 音轨");
		...
		if(k==total) {
			...
			if(this.groups.length>0){
				...
				if(this.custom_secondRow) this.groups[this.groups.length-1].secondRow = this.groups[this.groups.length-1].pl.Count+(this.groups[this.groups.length-1].pl.Count>1?" 音轨":" 音轨");
    ...
    this.populate = function(call_id, force_sorting,keep_showlist) {
		...
		console.log("--> GraphicScreensaver 填充排序:"+this.currently_sorted+" call_id:"+call_id)
        ...
        switch(event) {
            case "lbtn_down":
                ...
                if(this.ishover && this.rowsCount == 0 && LibraryItems_counter==0) {
					...
					if(this.activeIndex > -1 && !properties.expandInPlace){
						...
						if(fb.IsPaused) fb.Stop();
						fb.RunContextCommandWithMetadb("添加到播放队列", this.groups_draw[this.activeIndex].pl[0]);
						fb.Play();
					}
            ...
            case "lbtn_dblclk":
                ...
                if(a > -1){
					...
					if(fb.IsPaused) fb.Stop();
                    fb.RunContextCommandWithMetadb("添加到播放队列", this.groups_draw[a].pl[0]);
                    fb.Play();
                ...
                fb.RunContextCommandWithMetadb("播放", this.groups_draw[a].metadb);
                */
                break;
            case "mbtn_down":
                if(this.activeIndex > -1){
					fb.RunContextCommandWithMetadb("属性",this.groups_draw[this.activeIndex].pl);
                }
                break;
            case "move":
				this.setActiveRow(x,y);
...
function on_mouse_rbtn_down(x, y){
    ...
    if(!g_dragA && !g_dragR && wh > 10) {
        ...
        if(check__ > -1) {
			...
			sendTo.AppendMenuItem(MF_STRING, 3000, "新播放列表...");
		...
		if(!track_clicked && !album_clicked){
			_menu.AppendMenuItem(MF_STRING, 35, "音轨属性");
			drawSeparator = true;
		}
