...
function setButtons(){
	buttons = {
		Pause: new SimpleButton(ww/2-images.pause_img.Width/2,wh/2-images.pause_img.Height/2, images.pause_img.Width, 74, "Pause", "恢复播放", function () {
			fb.PlayOrPause();
		},function () {
			fb.Pause();
		},images.pause_img,images.pause_img),
		Play: new SimpleButton(ww/2-images.pause_img.Width/2,wh/2-images.pause_img.Height/2, images.pause_img.Width, 74, "Play", "点击播放", function () {
			if(g_infos.tracklist){ 
				...
				fb.RunContextCommandWithMetadb("添加到播放队列", g_cover.metadb);
				fb.Play();
			}
		},function () {},images.play_img,images.play_img),
		NowPlaying: new SimpleButton(ww/2-images.pause_img.Width/2,wh/2-images.pause_img.Height/2, images.pause_img.Width, 74, "NowPlaying", "显示正在播放", function () {
		...
		Random: new SimpleButton( ww/2-images.pause_img.Width/2, wh/2-images.pause_img.Height/2, 74, 74, "Random", "随机播放", function () {
...
function SimpleButton(x, y, w, h, text, tooltip_text, fonClick, fonDbleClick, N_img, H_img, state) {
	...
	this.setButtonState = function () {
        if(this.text=='Random'){
			...
			this.tooltip_text = fb.IsPaused?"恢复播放":"暂停播放";
	...
	if(fb.IsPlaying) {
		switch(true){
			...
			case (properties.dble_click_action==3):
				fb.RunContextCommandWithMetadb("打开文件夹", fb.GetNowPlaying(), 8);
			break;
			...
			case (properties.dble_click_action==5):
				if (g_infos.metadb && TextBtn_info.isXYInButton(x, y)) {
					fb.RunContextCommandWithMetadb("属性", g_infos.metadb);
				}
			break;
		}
	}
...
function on_drag_over(action, x, y, mask) {
	...
	try{
		if(fb.IsPlaying || fb.IsPaused) action.Text = "下一首";
		else action.Text = "播放";
	} catch(e){}
...
function on_mouse_rbtn_up(x, y){
	...
	main_menu.AppendMenuItem(MF_STRING, 35, "设置...");
	main_menu.AppendMenuSeparator();
	if(g_cover.isValid()){
		var now_playing_track = fb.GetNowPlaying();
		main_menu.AppendMenuItem(MF_STRING, 1, "打开完整/原始封面");
		main_menu.AppendMenuItem(MF_STRING, 6, "打开文件夹");
		main_menu.AppendMenuItem(MF_STRING, 8, "刷新此图像");
		...
		quickSearchMenu.AppendTo(main_menu, MF_STRING, "快速搜索...");
	}
	
	main_menu.AppendMenuSeparator();
	main_menu.AppendMenuItem(MF_STRING, 9, "显示正在播放");
	main_menu.AppendMenuSeparator();
	main_menu.AppendMenuItem(MF_STRING, 2, "属性");
	if(utils.IsKeyPressed(VK_SHIFT)) {
		main_menu.AppendMenuSeparator();
		main_menu.AppendMenuItem(MF_STRING, 100, "属性 ");
		main_menu.AppendMenuItem(MF_STRING, 101, "配置...");
		main_menu.AppendMenuSeparator();
		main_menu.AppendMenuItem(MF_STRING, 102, "重载入");
	}
	...
	switch(true) {
		...
		case (idx == 7):
			...
			break;
		case (idx == 6):
			fb.RunContextCommandWithMetadb("打开文件夹", now_playing_track, 8);
			break;
		case (idx == 8):
...
function draw_settings_menu(x,y){
		...
		_menu.AppendMenuItem(MF_STRING, 10, "封面总是跟随光标");
		_menu.CheckMenuItem(10,properties.follow_cursor);
		_menu.AppendMenuItem(MF_STRING, 11, "圆形封面");
		_menu.CheckMenuItem(11,properties.circleMode);
		_menu.AppendMenuItem(MF_STRING, 12, "保持比例")
		_menu.CheckMenuItem(12,globalProperties.keepProportion);
		_menu.AppendMenuItem(MF_STRING, 13, "填充整个空间");
		_menu.AppendMenuItem(MF_STRING, 20, "禁用这个封面缓存");
		...
		_menu.AppendMenuItem(MF_STRING, 18, "显示音轨细节");
		_menu.CheckMenuItem(18, trackinfostext_state.isActive());		
		_menu.AppendMenuItem(trackinfostext_state.isActive()?MF_STRING:MF_GRAYED, 17, "显示 2 行细节");
		_menu.CheckMenuItem(17, properties.doubleRowText);
		_menu.AppendMenuItem(trackinfostext_state.isActive()?MF_STRING:MF_GRAYED, 19, "编辑显示信息");	
		_menu.CheckMenuItem(19, properties.customInfos!="");
		_menu.AppendMenuItem(trackinfostext_state.isActive()?MF_STRING:MF_GRAYED, 16, "显示评级");
		_menu.CheckMenuItem(16,properties.showRating);		
		_menu.AppendMenuSeparator();

		var _single_click_menu = window.CreatePopupMenu();
		_single_click_menu.AppendMenuItem(MF_STRING, 14, "播放/暂停");
		_single_click_menu.AppendMenuItem(MF_STRING, 15, "显示正在播放");
		_single_click_menu.CheckMenuRadioItem(14, 15, 14+properties.single_click_action);
		_single_click_menu.AppendTo(_menu, MF_STRING, "封面按钮");

		var _dble_click_menu = window.CreatePopupMenu();
		_dble_click_menu.AppendMenuItem(MF_STRING, 3, "暂停播放");
		_dble_click_menu.AppendMenuItem(MF_STRING, 4, "在全部面板上显示正在播放");
		_dble_click_menu.AppendMenuItem(MF_STRING, 5, "打开完整/原始封面");
		_dble_click_menu.AppendMenuItem(MF_STRING, 6, "打开文件夹");
		_dble_click_menu.AppendMenuItem(MF_STRING, 7, "激活/退出 Mini 播放器");
		_dble_click_menu.AppendMenuItem(MF_STRING, 8, "编辑音轨属性");		
		_dble_click_menu.CheckMenuRadioItem(3, 8, 3+properties.dble_click_action);
		_dble_click_menu.AppendTo(_menu, MF_STRING, "双击动作");

		wallpapper_menu.AppendMenuItem(MF_STRING, 200, "启用");
		wallpapper_menu.CheckMenuItem(200, properties.showwallpaper);
		wallpapper_menu.AppendMenuItem(MF_STRING, 220, "模糊");
		wallpapper_menu.CheckMenuItem(220, properties.wallpaperblurred);
		wallpapper_menub.AppendMenuItem(MF_STRING, 221, "填充");
		wallpapper_menub.CheckMenuItem(221, properties.wallpaperdisplay==0);
		wallpapper_menub.AppendMenuItem(MF_STRING, 222, "适合");
		wallpapper_menub.CheckMenuItem(222, properties.wallpaperdisplay==1);
		wallpapper_menub.AppendMenuItem(MF_STRING, 223, "拉伸");
		wallpapper_menub.CheckMenuItem(223, properties.wallpaperdisplay==2);
		wallpapper_menub.AppendTo(wallpapper_menu,MF_STRING, "壁纸大小");
		wallpapper_menu.AppendTo(_menu,MF_STRING, "壁纸背景");
        ...
        switch(true) {
			...
			case (idx == 19): 
					...
					customNowPlayingInfos("编辑显示信息"
										,"<div class='titleBig'>编辑显示信息</div><div class='separator'></div><br/>为每行显示的信息输入一个标题格式化脚本. 对于默认信息, 两行留空. 默认的信息是:\n\n     行 1: %artist%  |  %album% (%date%)\n     行 2: %codec% | %codec_profile% | %bitrate%K | %play_count%\n\n您可以在这里使用完整的 foobar2000 标题格式语法.<br/><a href=\"http://tinyurl.com/lwhay6f\" target=\"_blank\">点击这里</a> 了解关于工具栏标题格式信息. (http://tinyurl.com/lwhay6f)<br/>"
										,''
										,'第一行:##第二行:'
