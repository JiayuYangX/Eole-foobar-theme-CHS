...
function build_buttons(){
	if(btn_initialized){
		...
		buttons = {
			//LyricsWidth: new JSButton(0, 0, 40, images.lyrics_off_icon.Height, "简介", "简介", "显示简介", function () {
...
function on_paint(gr) {
	...
	if(lyricsText_Width<0) {
		lyricsText_Width = gr.CalcTextWidth("歌词", font_title)+10;
		positionButtons();
	}
	gr.GdiDrawText("歌词", font_title, colors.normal_txt, padding_left, (lyrics_state.isEqual(5)?padding_top_nobio:padding_top), ww - padding_left-padding_right, header_height, DT_TOP | DT_LEFT | DT_END_ELLIPSIS | DT_NOPREFIX);
	drawAllButtons(gr);
}
...
function on_mouse_rbtn_up(x, y){
	...
	_menu.AppendMenuItem(MF_STRING, 99, "始终深色布局");
